...
// headless×business) for the subset of capabilities where the matrix in
...
// brief names them: N/H for engine (noise/headless), P/B for account
...
func decideQuadrant(t *testing.T, r *CapabilityRegistry, capability domain.Capability) quadrant {
	...
	if err != nil {
		t.Fatalf("Decide(%q, noise, personal): %v", capability, err)
	}
	nb, err := r.Decide(capability, domain.EngineNoise, domain.AccountTypeBusiness)
	if err != nil {
		t.Fatalf("Decide(%q, noise, business): %v", capability, err)
	}
	hp, err := r.Decide(capability, domain.EngineHeadless, domain.AccountTypePersonal)
	if err != nil {
		t.Fatalf("Decide(%q, headless, personal): %v", capability, err)
	}
	hb, err := r.Decide(capability, domain.EngineHeadless, domain.AccountTypeBusiness)
	if err != nil {
		t.Fatalf("Decide(%q, headless, business): %v", capability, err)
	}
	return quadrant{np, nb, hp, hb}
}

// TestQuadrant_SendCarousel: noise supported+confirmed (HOUSEKEEP F216,
// device-photographed), headless unknown (no adapter found at all).
...
// cell (H140, cited in pkg/application/contracts/group_ports.go). noise
// supported, headless confirmed engine_unsupported on both account types.
...
// message creation" row — headless/messenger only covers actions on
...
// TestQuadrant_StarMessage: noise supported via app-state patch
...
// row's note); headless unknown (no adapter). Recorded separately from
...
// noise AND headless), used here to lock the opposite pattern from the
// engine-gap tests above: NP=NB=HP=HB (all supported/probable). The
// headless side is "local ack only, receipt-to-sender NOT confirmed"
...
// SAME status (supported on both) but DIFFERENT evidence — noise is
// EvidenceConfirmed (unit-tested against a real usync certificate fixture),
// headless is EvidenceProbable (reuses a getter measured live on the lab
...
func TestQuadrant_DetectAccountType(t *testing.T) {
	...
	for name, d := range map[string]CapabilityDecision{"NP": q.np, "NB": q.nb} {
		if !d.Supported || d.Status != domain.StatusSupported || d.Evidence != domain.EvidenceNotTested {
			t.Fatalf("%s: got Supported=%v Status=%v Evidence=%v, want supported on noise (not_tested per account type; unknown-type evidence was confirmed)", name, d.Supported, d.Status, d.Evidence)
		}
	}
	for name, d := range map[string]CapabilityDecision{"HP": q.hp, "HB": q.hb} {
		if !d.Supported || d.Status != domain.StatusSupported || d.Evidence != domain.EvidenceNotTested {
			t.Fatalf("%s: got Supported=%v Status=%v Evidence=%v, want supported on headless (not_tested per account type; unknown-type evidence was probable)", name, d.Supported, d.Status, d.Evidence)
		}
	}
}

// TestQuadrant_CheckPairingStatus: noise supported, headless unknown —
...
// TestQuadrant_SetGroupPhoto on purpose: same shape of "noise yes,
// headless no" but this one must stay Unknown, not EngineUnsupported,
...
// headless side carries a documented asymmetry (H58/H65: the write can
...
// downgrades headless to reflect the read-back gap would need to update
...
func TestQuadrant_AccountTypeNeverChangesStatusToday(t *testing.T) {
	...
	for _, capability := range sample {
		...
		if q.np.Status != q.nb.Status || q.np.Supported != q.nb.Supported {
			t.Fatalf("%s: personal/business diverge on noise (NP=%v/%v NB=%v/%v) — account_type now carries signal; update items 76-79 classification instead of this test", capability, q.np.Status, q.np.Supported, q.nb.Status, q.nb.Supported)
		}
		if q.hp.Status != q.hb.Status || q.hp.Supported != q.hb.Supported {
			t.Fatalf("%s: personal/business diverge on headless (HP=%v/%v HB=%v/%v) — account_type now carries signal; update items 76-79 classification instead of this test", capability, q.hp.Status, q.hp.Supported, q.hb.Status, q.hb.Supported)
		}
	}
}
