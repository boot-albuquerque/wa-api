...
func initCustomHandlers(s *server) {
	...
	customHandlerSet = &customHandlers{
		...
		// Não há handler HTTP para GET /session/capabilities nesta worktree —
		// fora do escopo do porte de F273 (registry de pareamento). O
		// capabilityregistry em si está totalmente wireado e usável (ver
		// `capabilities` acima), só não está exposto como rota própria ainda.
	}
}
