...
func TestSemFallbackSILENCIOSO(t *testing.T) {
	casos := []struct {
		...
		{"socket serve tudo", domain.EngineNoise, false, false, false},
		{"socket serve tudo mesmo havendo headless", domain.EngineNoise, true, false, false},
		{"headless com implementação", domain.EngineWaHeadless, true, true, false},
		{"headless SEM implementação RECUSA", domain.EngineWaHeadless, false, false, true},
	}
	for _, c := range casos {
...
func TestARecusaDIZOQueAconteceu(t *testing.T) {
	err := error(ErrEngineSemPort{
		Port: "GroupPhotoSetter", Engine: domain.EngineWaHeadless, TxtID: "s1",
...
func TestSemMotivoARecusaAindaDizQueNaoHaFallback(t *testing.T) {
	msg := ErrEngineSemPort{Port: "X", Engine: domain.EngineWaHeadless}.Error()
