...
//     real, sobre um *noise.Client real construído com o bridge como
...
func TestWalogSeam_ErroDoSDKSaiSemWadebug(t *testing.T) {
	...
	bridge := logbridge.New(log.Logger, logbridge.ModuleClient, logbridge.ParseLevel(""))
	...
	if recs[0].str(logbridge.FieldModule) != "Client/Recv" {
		t.Errorf("%s = %v, queria \"Client/Recv\"", logbridge.FieldModule, recs[0].str(logbridge.FieldModule))
	}
	...
	appCtx.UserInfoCache.Set(logSeamUser, Values{M: map[string]string{
		"Id": logSeamUser, "Webhook": "", "Events": "", "Jid": "", "Name": "",
	}}, 0)
	t.Cleanup(func() { appCtx.UserInfoCache.Delete(logSeamUser) })

	sqlDB := schemaDB(t)
	seedUser(t, sqlDB, logSeamUser, logSeamToken, "")
	evh := &UserEventHandler{
		UserID:   logSeamUser,
		Token:    logSeamToken,
		DB:       sqlDB,
		WAClient: noise.NewClient(&store.Device{Log: bridge.Sub("Device")}, bridge),
	}
	...
	for _, rec := range recs {
		if rec.str("userID") == logSeamUser {
			sawSubscription = true
		}
	}
	if !sawSubscription {
		t.Errorf("nenhum registro do handler citou userID=%q: %v", logSeamUser, recs)
	}
}

const (
	logSeamUser  = "log-seam-user"
	logSeamToken = "log-seam-token"
)
