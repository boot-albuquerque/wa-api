...
type listerDuplo struct {
	lista waheadless.ChatList
	chat  waheadless.Chat
	err   error
}

func (l listerDuplo) List(context.Context, int, string) (waheadless.ChatList, error) {
	return l.lista, l.err
}
func (l listerDuplo) ByJID(context.Context, string, string) (waheadless.Chat, error) {
	return l.chat, l.err
}

type inviterDuplo struct {
	code waheadless.GroupInviteCode
	info waheadless.GroupInvite
	err  error
}

func (i inviterDuplo) InviteCode(context.Context, string, string) (waheadless.GroupInviteCode, error) {
	return i.code, i.err
}
func (i inviterDuplo) InviteInfo(context.Context, string, string) (waheadless.GroupInvite, error) {
	return i.info, i.err
}
...
func listaMista() waheadless.ChatList {
	return waheadless.ChatList{
		Chats: []waheadless.Chat{
...
func TestJIDDePessoaNaoPassaPorGrupo(t *testing.T) {
	d := com(listerDuplo{chat: waheadless.Chat{JID: "111@lid", IsGroup: false}}, nil)
...
func TestOLinkEPedidoEXPLICITAMENTE(t *testing.T) {
	got, err := com(nil, inviterDuplo{code: waheadless.GroupInviteCode{Code: "AbCdEf"}}).
