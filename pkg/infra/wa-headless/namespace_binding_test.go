...
// sufixos vêm de wa-api/internal/wa-headless e NÃO de spa/, porque alcançar o
...
func TestVocabularioCompartilhadoBateComAsConstantesReaisDaPagina(t *testing.T) {
	casos := []struct {
		...
		{waheadless.ServerPhone, domain.NamespacePhone},
		{waheadless.ServerLID, domain.NamespaceLID},
		{waheadless.ServerGroup, domain.NamespaceGroup},
	}
	for _, c := range casos {
...
func TestOsDoisTransportesConcordamNoSignificadoENaoNaGrafia(t *testing.T) {
	pagina := domain.JID("5511999999999" + waheadless.ServerPhone)
...
func TestFachadaExpoeARegraDeIdentidadeNaoResolvida(t *testing.T) {
	if !waheadless.IsUnresolvedIdentity("5511999999999" + waheadless.ServerPhone) {
		t.Error("identidade de telefone devia ser não-resolvida neste build")
	}
	if waheadless.IsUnresolvedIdentity("120363000000000" + waheadless.ServerGroup) {
		t.Error("grupo não é pessoa e não pode ser tratado como identidade não-resolvida")
	}
	if waheadless.IsUnresolvedIdentity("5511999999999" + waheadless.ServerLID) {
