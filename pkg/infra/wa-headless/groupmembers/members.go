...
type members interface {
	AddParticipant(ctx context.Context, groupJID, participantJID, label string) (waheadless.GroupMembership, error)
	RemoveParticipant(ctx context.Context, groupJID, participantJID, label string) (waheadless.GroupMembership, error)
}
...
func (m *Manager) UpdateGroupParticipants(ctx context.Context, txtID string, groupJID domain.JID, participants []domain.JID, action domain.ParticipantAction) (domain.ParticipantsUpdate, error) {
	if action != domain.ParticipantAdd && action != domain.ParticipantRemove {
		return domain.ParticipantsUpdate{}, fmt.Errorf("waheadless: unknown participant action %q", action)
	}
	if len(participants) == 0 {
		return domain.ParticipantsUpdate{}, fmt.Errorf("waheadless: no participants to update")
	}
	...
	for _, p := range participants {
		...
		var r waheadless.GroupMembership
