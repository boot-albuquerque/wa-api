...
type settings interface {
	SetSubject(ctx context.Context, groupJID, subject, label string) (waheadless.GroupRename, error)
	SetDescription(ctx context.Context, groupJID, description, label string) (waheadless.GroupDescribed, error)
...
func (m *Manager) SetGroupAnnounce(ctx context.Context, txtID string, group domain.JID, announce bool) error {
	return m.policy(ctx, txtID, group, waheadless.PolicyMessagesAdminsOnly, announce, announceLabel)
}
...
func (m *Manager) SetGroupLocked(ctx context.Context, txtID string, group domain.JID, locked bool) error {
	return m.policy(ctx, txtID, group, waheadless.PolicyInfoAdminsOnly, locked, lockedLabel)
}
...
func renameUnconfirmed(r waheadless.GroupRename, want string) error {
	if r.AlreadyInState || r.Field != "" {
		return nil
	}
	return fmt.Errorf("waheadless: nome pedido (%d bytes) sem campo do modelo que o confirme", len(want))
}
...
func describeUnconfirmed(d waheadless.GroupDescribed, want string) error {
	if strings.TrimSpace(d.Text) == strings.TrimSpace(want) {
		return nil
	}
	return fmt.Errorf("waheadless: descricao pedida (%d bytes) relida diferente (%d bytes, campo %q)",
		len(want), len(d.Text), d.Source)
}

// policyUnconfirmed refuses a policy change the page did not read back.
func policyUnconfirmed(c waheadless.GroupPolicyChange) error {
	if c.NoOp || c.Verified {
		return nil
	}
	return fmt.Errorf("waheadless: politica %q pedida (%t) sem confirmacao lida de volta", c.Policy, c.Wanted)
}

func (m *Manager) policy(ctx context.Context, txtID string, group domain.JID, p waheadless.GroupPolicy, on bool, label string) error {
	s, pageJID, err := m.prepare(ctx, txtID, group)
	if err != nil {
		return err
	}
