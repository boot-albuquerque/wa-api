...
// read by QRReader, matching the historical wa_noise contract
...
//
// wa_noise's equivalent (waNoiseSessionStarter.CheckOwnership,
// pairing_providers.go) claims a cross-replica LEASE, because the socket
// transport can be driven from any replica that holds the lease. wa_headless
...
//
// Fire-and-forget by contract, same as wa_noise's Starter
	...
	go func() {
		if _, err := s.sessions.EvaluatorForPairing(context.Background(), txtID); err != nil {
			log.Warn().Err(err).Str("txt_id", txtID).
				Msg("wa_headless: pairing session failed to start")
		}
	}()
}
