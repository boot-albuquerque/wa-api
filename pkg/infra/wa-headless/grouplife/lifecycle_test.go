...
type duplo struct {
	criado waheadless.GroupCreated
	entrou waheadless.GroupJoined
	jids   []string
	saiuDe string
	err    error
}

func (d *duplo) Ensure(_ context.Context, _ string, participants []string, _ string) (waheadless.GroupCreated, error) {
	d.jids = participants
	return d.criado, d.err
}
func (d *duplo) JoinByInvite(context.Context, string, string) (waheadless.GroupJoined, error) {
...
func TestEntradaPENDENTENaoViraAdesao(t *testing.T) {
	d := &duplo{entrou: waheadless.GroupJoined{Pending: true}}

	got, err := com(d).JoinGroup(context.Background(), "s1", "AbCdEf")
	if err != nil {
		t.Fatalf("JoinGroup: %v", err)
	}
	j, ok := got.(waheadless.GroupJoined)
	if !ok {
