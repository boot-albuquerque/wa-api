...
func TestNoiseSession_Connect_RequiresCredentials(t *testing.T) {
	s := &noiseSession{device: &store.Device{}, client: &fakeSessionClient{}}
	if err := s.Connect(context.Background()); err == nil {
		t.Error("expected error connecting without credentials")
	}
}

func TestNoiseSession_Connect_Success(t *testing.T) {
	jid := types.NewJID("5511999999999", types.DefaultUserServer)
	called := false
	client := &fakeSessionClient{ConnectFn: func() error { called = true; return nil }}
	s := &noiseSession{device: &store.Device{ID: &jid}, client: client}
	if err := s.Connect(context.Background()); err != nil {
...
func TestNoiseSession_DisconnectIsConnectedIsLoggedIn(t *testing.T) {
	disconnected := false
...
func TestNoiseSession_Logout(t *testing.T) {
	...
	s2 := &noiseSession{device: &store.Device{}, client: failing}
	if err := s2.Logout(context.Background()); err == nil {
		t.Error("expected error from failing Logout")
	}
}

func TestNoiseSession_SetProxy(t *testing.T) {
	...
	t.Run("http mode", func(t *testing.T) {
		var gotAddr string
		client := &fakeSessionClient{SetProxyAddressFn: func(addr string, opts ...noise.SetProxyOptions) error {
			gotAddr = addr
			return nil
		}}
		s := &noiseSession{device: &store.Device{}, client: client}
	...
	t.Run("socks5 mode", func(t *testing.T) {
		called := false
		client := &fakeSessionClient{SetSOCKSProxyFn: func(px proxy.Dialer, opts ...noise.SetProxyOptions) {
			called = true
		}}
		s := &noiseSession{device: &store.Device{}, client: client}
