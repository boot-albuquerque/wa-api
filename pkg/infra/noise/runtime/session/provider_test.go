...
type fakeSessionClient struct {
	...
	AddEventHandlerFn func(handler noise.EventHandler) uint32
	RemoveHandlerFn   func(id uint32) bool
	SetSOCKSProxyFn   func(px proxy.Dialer, opts ...noise.SetProxyOptions)
	SetProxyAddressFn func(addr string, opts ...noise.SetProxyOptions) error
}

func (f *fakeSessionClient) GetQRChannel(ctx context.Context) (<-chan noise.QRChannelItem, error) {
...
func (f *fakeSessionClient) AddEventHandler(handler noise.EventHandler) uint32 {
...
func (f *fakeSessionClient) SetSOCKSProxy(px proxy.Dialer, opts ...noise.SetProxyOptions) {
	if f.SetSOCKSProxyFn != nil {
		f.SetSOCKSProxyFn(px, opts...)
	}
}
func (f *fakeSessionClient) SetProxyAddress(addr string, opts ...noise.SetProxyOptions) error {
...
func TestNewSessionProviderAdapter_DefaultCallback_EnablesRetryStore(t *testing.T) {
	...
	cli := sess.(*noiseSession).NoiseClient()
	if cli == nil {
		t.Fatal("NoiseClient returned nil for default callback")
	}
...
func TestNewSessionProviderWithLogger_EnablesRetryStore(t *testing.T) {
	...
	cli := sess.(*noiseSession).NoiseClient()
	if cli == nil {
		t.Fatal("NoiseClient returned nil for logger callback")
	}
	if !cli.UseRetryMessageStore {
		t.Error("UseRetryMessageStore is false; the durable retry store must be enabled (F214)")
	}
...
var (
	...
	_ appport.Session         = (*noiseSession)(nil)
)
