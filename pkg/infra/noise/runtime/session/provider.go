...
// pelo mesmo motivo de client.Client em wa_client_seam.go.
type deviceContainer interface {
	GetDevice(ctx context.Context, jid types.JID) (*store.Device, error)
	NewDevice() *store.Device
}

// sessionClient é a superfície de *noise.Client que uma Session exercita.
...
type sessionClient interface {
	GetQRChannel(ctx context.Context) (<-chan noise.QRChannelItem, error)
	Connect() error
	Disconnect()
	IsConnected() bool
	IsLoggedIn() bool
	Logout(ctx context.Context) error
	AddEventHandler(handler noise.EventHandler) uint32
	RemoveEventHandler(id uint32) bool
	SetSOCKSProxy(px proxy.Dialer, opts ...noise.SetProxyOptions)
	SetProxyAddress(addr string, opts ...noise.SetProxyOptions) error
}
...
// SessionProviderAdapter implementa appport.SessionProvider sobre o noise,
...
// construção do cliente; quando nil, usa noise.NewClient sem logger.
...
// log, como noise.NewClient(dev, nil).
func NewSessionProviderWithLogger(container deviceContainer, lookupJID DeviceJIDLookup, logger sdklog.Logger) *SessionProviderAdapter {
	return NewSessionProviderAdapter(container, lookupJID, func(dev *store.Device) sessionClient {
		cli := noise.NewClient(dev, logger)
		cli.UseRetryMessageStore = true
		return cli
	})
}
