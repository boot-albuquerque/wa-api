...
func (JIDResolverAdapter) ResolveJID(_ context.Context, raw string) (domain.JID, error) {
	jid, ok := ParseJID(raw)
	if !ok {
		return "", fmt.Errorf("noise: could not parse JID %q", raw)
	}
...
func (JIDResolverAdapter) ResolveQualifiedJID(_ context.Context, raw string) (domain.JID, error) {
	...
	if err != nil {
		return "", fmt.Errorf("noise: could not parse JID %q: %w", raw, err)
	}
	...
	// existe para impedir.
	if jid.User == "" {
		return "", fmt.Errorf("noise: JID %q has no server", raw)
	}
	return domain.JID(jid.String()), nil
}
...
func ToJID(j domain.JID) (types.JID, error) {
	...
	if !ok {
		return types.JID{}, fmt.Errorf("noise: could not parse JID %q", string(j))
	}
	return parsed, nil
}
