// Package noise é apenas a âncora de documentação do diretório: todo o
...
// internal/noise/. A distinção com aquela árvore é deliberada e a
// repetição de nomes entre as duas é informativa, não redundante:
//
//	internal/noise/capabilities/group   implementa a capability Group do protocolo
//	pkg/infra/noise/adapters/group      faz a aplicação usar Groups via WhatsApp
//
...
// descuido: "waclient" no CAMINHO é redundante (já estamos sob noise/),
// mas o identificador aparece em ~38 arquivos onde esse caminho não está à
// vista, e lá `client.Client` carrega o que `client.Client` — um stutter —
...
//
//	client/          client.Client (a superfície de *noise.Client que os
//	                 adapters exercitam), o getter por userID, a ponte para o
//	                 tipo concreto do SDK e os timeouts de fronteira
//	client/testkit/  fakes exportados de client.Client e do ContactStore,
...
//	walog/   ponte log.Logger -> zerolog usada pelo SDK (logging DO PROTOCOLO)
//
