...
type fakeCM struct {
	clients []*noise.Client
}

func (f *fakeCM) GetNoiseClientsCount() int { return len(f.clients) }
func (f *fakeCM) IterateNoiseClients(cb func(*noise.Client) bool) {
...
func TestSessionCounterAdapter_CountSessions_NilClientsEntries(t *testing.T) {
	cm := &fakeCM{clients: []*noise.Client{nil}}
	a := NewSessionCounterAdapter(cm)
	counts, err := a.CountSessions(context.Background())
	if err != nil {
		t.Fatalf("CountSessions = %v", err)
