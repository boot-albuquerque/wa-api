...
import (
	...
	"strings"
	"time"
	"wa-api/pkg/infra/noise/adapters/profile"
...
func NewMiscAdapter(getClient clientpkg.Getter) *MiscAdapter {
...
func (a *MiscAdapter) RequestUnavailableMessage(ctx context.Context, txtID string, chat, sender domain.JID, messageID string) (domain.UnavailableMessageAck, error) {
	...
	resp, err := client.SendMessage(ctxWithTimeout, chatJID, unavailableMessage, noise.SendRequestExtra{Peer: true})
	if err != nil {
		return domain.UnavailableMessageAck{}, err
	}
...
func (a *MiscAdapter) CreateNewsletter(ctx context.Context, txtID, name, description string, picture []byte) (*domain.NewsletterMetadata, error) {
	...
	ctxWithTimeout, cancel := context.WithTimeout(ctx, clientpkg.RequestTimeout)
	defer cancel()
	created, err := client.CreateNewsletter(ctxWithTimeout, noise.CreateNewsletterParams{
...
func (a *MiscAdapter) NewsletterMessages(ctx context.Context, txtID string, jid domain.JID, count int, before string) ([]domain.NewsletterMessage, error) {
	...
	ctxWithTimeout, cancel := context.WithTimeout(ctx, clientpkg.RequestTimeout)
	defer cancel()
	msgs, err := client.GetNewsletterMessages(ctxWithTimeout, parsed, &noise.GetNewsletterMessagesParams{
...
func (a *MiscAdapter) NewsletterMessageUpdates(ctx context.Context, txtID string, jid domain.JID, count int, since time.Time, after string) ([]domain.NewsletterMessage, error) {
	...
	ctxWithTimeout, cancel := context.WithTimeout(ctx, clientpkg.RequestTimeout)
	defer cancel()
	updates, err := client.GetNewsletterMessageUpdates(ctxWithTimeout, parsed, &noise.GetNewsletterUpdatesParams{
...
func resolveToLID(ctx context.Context, client clientpkg.Client, jid types.JID, operation string) types.JID {
...
func (a *MiscAdapter) clientAndJID(txtID string, jid domain.JID) (clientpkg.Client, types.JID, error) {
	client, err := a.Client(txtID)
	if err != nil {
