...
func TestMiscAdapter_ProfileAccess_OK(t *testing.T) {
	fake := &testkit.Fake{StoreFn: func() *store.Device { return &store.Device{} }}
...
func TestMiscAdapter_RequestUnavailableMessage_PropagatesError(t *testing.T) {
	...
	fake := &testkit.Fake{
		...
		SendMessageFn: func(ctx context.Context, to types.JID, msg *waE2E.Message, extra ...noise.SendRequestExtra) (noise.SendResponse, error) {
...
func TestMiscAdapter_RejectCall_PropagatesError(t *testing.T) {
	sdkErr := errors.New("boom")
	fake := &testkit.Fake{RejectCallFn: func(ctx context.Context, callFrom types.JID, callID string) error { return sdkErr }}
	a := NewMiscAdapter(testkit.GetterWith(map[string]client.Client{"u1": fake}))
