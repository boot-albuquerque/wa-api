...
func TestProfileDataAccess_NewProfileDataAccess(t *testing.T) {
	da := NewProfileDataAccess(&noise.Client{})
	if da == nil {
...
func TestProfileDataAccess_PushName_WithStore(t *testing.T) {
	da := &ProfileDataAccess{client: &noise.Client{Store: &store.Device{PushName: "Alice"}}}
...
func TestProfileDataAccess_OwnJID_OK(t *testing.T) {
	...
	da := &ProfileDataAccess{client: &noise.Client{Store: &store.Device{ID: &myJID}}}
	got, ok := da.OwnJID()
	if !ok {
