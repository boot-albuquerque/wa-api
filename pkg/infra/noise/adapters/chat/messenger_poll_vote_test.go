...
func TestChatMessengerAdapter_SendPollVote_SendsToRecipient(t *testing.T) {
	...
	f := &testkit.Fake{
		...
		SendMessageFn: func(_ context.Context, to types.JID, _ *waE2E.Message, _ ...noise.SendRequestExtra) (noise.SendResponse, error) {
			sentTo = to
			return noise.SendResponse{
...
func TestChatMessengerAdapter_SendPollVote_ClientSuppliedIDForwarded(t *testing.T) {
	...
	f := &testkit.Fake{
		...
		SendMessageFn: func(_ context.Context, _ types.JID, _ *waE2E.Message, extra ...noise.SendRequestExtra) (noise.SendResponse, error) {
			gotExtra = extra
			return noise.SendResponse{ID: "server-id"}, nil
		},
	}
	a := pollVoteAdapter(f)
