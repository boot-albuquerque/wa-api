...
func pollAdapter(f *testkit.Fake, rec PollOptionRecorder) *ChatMessengerAdapter {
	return NewChatMessengerAdapter(testkit.GetterWith(map[string]client.Client{"u1": f})).WithPollOptions(rec)
}
...
func TestChatMessengerAdapter_SendPoll_SelectableOptionCountIsOne(t *testing.T) {
	...
	f := &testkit.Fake{
		...
		SendMessageFn: func(context.Context, types.JID, *waE2E.Message, ...noise.SendRequestExtra) (noise.SendResponse, error) {
			return noise.SendResponse{ID: "poll-wire-id", Timestamp: time.Unix(1755500110, 0)}, nil
		},
	}
...
func TestChatMessengerAdapter_SendPoll_RemembersOptionsUnderServerID(t *testing.T) {
	f := &testkit.Fake{
		SendMessageFn: func(context.Context, types.JID, *waE2E.Message, ...noise.SendRequestExtra) (noise.SendResponse, error) {
			return noise.SendResponse{ID: "id-que-o-sdk-usou", Timestamp: time.Unix(1755500110, 0)}, nil
		},
	}
	rec := &pollRecorder{}
	a := pollAdapter(f, rec)
...
func TestChatMessengerAdapter_SendPoll_StoredOptionsResolveTheVoteHashes(t *testing.T) {
	f := &testkit.Fake{
		SendMessageFn: func(context.Context, types.JID, *waE2E.Message, ...noise.SendRequestExtra) (noise.SendResponse, error) {
			return noise.SendResponse{ID: "poll-msg-1", Timestamp: time.Unix(1755500110, 0)}, nil
		},
	}
	rec := &pollRecorder{}
	a := pollAdapter(f, rec)
...
func TestChatMessengerAdapter_SendPoll_ClientIDIsForwarded(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			f := &testkit.Fake{
				SendMessageFn: func(_ context.Context, _ types.JID, _ *waE2E.Message, extra ...noise.SendRequestExtra) (noise.SendResponse, error) {
					gotExtra = extra
					return noise.SendResponse{ID: "poll-wire-id"}, nil
				},
			}
			a := pollAdapter(f, &pollRecorder{})
...
func TestChatMessengerAdapter_SendPoll_WithoutRecorderRefusesToSend(t *testing.T) {
	...
	f := &testkit.Fake{
		SendMessageFn: func(context.Context, types.JID, *waE2E.Message, ...noise.SendRequestExtra) (noise.SendResponse, error) {
			sendCalled = true
			return noise.SendResponse{ID: "nao-devia-chegar-aqui"}, nil
		},
	}
