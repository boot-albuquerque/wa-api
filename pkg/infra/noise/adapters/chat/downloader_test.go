...
func readDownloadable(t *testing.T, msg noise.DownloadableMessage) downloadableFields {
	t.Helper()
	switch m := msg.(type) {
...
func TestMediaDownloaderAdapter_BuildsTheRightProtobuf(t *testing.T) {
	for _, c := range downloadKindCases() {
		t.Run(c.capability, func(t *testing.T) {
			...
			var seen noise.DownloadableMessage
			calls := 0

			fake := &testkit.Fake{
				DownloadFn: func(_ context.Context, msg noise.DownloadableMessage) ([]byte, error) {
					calls++
					seen = msg
					return want, nil
				},
			}
			a := NewMediaDownloaderAdapter(testkit.GetterWith(map[string]client.Client{"u1": fake}))
...
func TestMediaDownloaderAdapter_SDKErrorPropagates(t *testing.T) {
	...
	fake := &testkit.Fake{
		DownloadFn: func(context.Context, noise.DownloadableMessage) ([]byte, error) {
			return nil, sentinel
		},
	}
	a := NewMediaDownloaderAdapter(testkit.GetterWith(map[string]client.Client{"u1": fake}))
