...
func sendButtonsCapturing(t *testing.T, payload domain.ButtonsPayload, _ *domain.ReplyContext, id string) (*waE2E.Message, []noise.SendRequestExtra) {
	t.Helper()
	sent, extra, _ := sendButtonsCapturingWithFake(t, &testkit.Fake{}, payload, nil, nil, id)
	return sent, extra
}
...
// ele.
func sendButtonsCapturingWithFake(t *testing.T, f *testkit.Fake, payload domain.ButtonsPayload, _ *domain.ReplyContext, mentionedJID []string, id string) (*waE2E.Message, []noise.SendRequestExtra, error) {
	t.Helper()

	var sent *waE2E.Message
	var gotExtra []noise.SendRequestExtra
	f.SendMessageFn = func(_ context.Context, _ types.JID, m *waE2E.Message, extra ...noise.SendRequestExtra) (noise.SendResponse, error) {
		sent, gotExtra = m, extra
		return noise.SendResponse{ID: "buttons-wire-id", Timestamp: time.Unix(1755500140, 0)}, nil
	}

	_, err := buttonsAdapter(f).SendButtons(context.Background(), "u1", buttonsChatJID, payload, nil, mentionedJID, id)
...
func TestChatMessengerAdapter_SendButtons_NoSession(t *testing.T) {
	...
	_, err := a.SendButtons(context.Background(), "u1", buttonsChatJID,
		domain.ButtonsPayload{Body: "corpo"}, nil, nil, "")
...
// payload, o adapter sobe a imagem com noise.MediaImage e monta o header
...
func TestChatMessengerAdapter_SendButtons_HeaderImageIsUploaded(t *testing.T) {
	...
	f := &testkit.Fake{
		UploadFn: func(_ context.Context, plaintext []byte, appInfo noise.MediaType) (noise.UploadResponse, error) {
	...
	if gotAppInfo != noise.MediaImage {
		t.Errorf("appInfo do Upload = %v, quero noise.MediaImage", gotAppInfo)
	}
...
func TestChatMessengerAdapter_SendButtons_UploadFailureNeverSends(t *testing.T) {
	...
	sendCalled := false
	f := &testkit.Fake{
		UploadFn: func(context.Context, []byte, noise.MediaType) (noise.UploadResponse, error) {
...
func TestChatMessengerAdapter_SendButtons_WithReplyTo(t *testing.T) {
	...
	f := &testkit.Fake{
		SendMessageFn: func(_ context.Context, _ types.JID, m *waE2E.Message, _ ...noise.SendRequestExtra) (noise.SendResponse, error) {
			gotMsg = m
			return noise.SendResponse{ID: "wire-reply-btn"}, nil
		},
	}
	...
	_, err := buttonsAdapter(f).SendButtons(context.Background(), "u1", buttonsChatJID, payload, reply, nil, "")
	if err != nil {
		t.Fatalf("SendButtons = %v", err)
	}
