...
func noiseCreateParams() noise.CreateNewsletterParams {
	return noise.CreateNewsletterParams{Name: "n"}
}
...
func TestFakeNewsletter_SemFuncaoInjetadaDevolveZeroSemPanico(t *testing.T) {
	...
	casos := map[string]func() error{
		"CreateNewsletter":               func() error { _, err := f.CreateNewsletter(ctx, noiseCreateParams()); return err },
