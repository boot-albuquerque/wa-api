...
// Client é a superfície mínima de *noise.Client exercitada pelos
...
//
// *noise.Client satisfaz esta interface por construção: cada método
...
type Client interface {
	...
	Upload(ctx context.Context, plaintext []byte, appInfo noise.MediaType) (noise.UploadResponse, error)
	...
	Download(ctx context.Context, msg noise.DownloadableMessage) ([]byte, error)
	...
	CreateGroup(ctx context.Context, req noise.ReqCreateGroup) (*types.GroupInfo, error)
	...
	UpdateGroupParticipants(ctx context.Context, jid types.JID, participantChanges []types.JID, action noise.ParticipantChange) ([]types.GroupParticipant, error)
	GetGroupRequestParticipants(ctx context.Context, jid types.JID) ([]types.GroupParticipantRequest, error)
	UpdateGroupRequestParticipants(ctx context.Context, jid types.JID, participantChanges []types.JID, action noise.ParticipantRequestChange) ([]types.GroupParticipant, error)
	...
	GetProfilePictureInfo(ctx context.Context, jid types.JID, params *noise.GetProfilePictureParams) (*types.ProfilePictureInfo, error)
	...
	SendPeerMessage(ctx context.Context, message *waE2E.Message) (noise.SendResponse, error)
	...
	CreateNewsletter(ctx context.Context, params noise.CreateNewsletterParams) (*types.NewsletterMetadata, error)
	...
	GetNewsletterMessages(ctx context.Context, jid types.JID, params *noise.GetNewsletterMessagesParams) ([]*types.NewsletterMessage, error)
	GetNewsletterMessageUpdates(ctx context.Context, jid types.JID, params *noise.GetNewsletterUpdatesParams) ([]*types.NewsletterMessage, error)
	...
	NewsletterCreateAdminInvite(ctx context.Context, channelJID, userJID types.JID) (noise.NewsletterAdminInvite, error)
...
// Em produção é clientManager.GetNoiseClient; nos testes é uma função
// controlada pelo caso.
type Getter func(txtID string) Client

// RealClient adapta *noise.Client para a interface Client. O método
...
// ClientForGetter converte o getter de produção (devolve *noise.Client)
...
func ClientForGetter(getConcrete func(txtID string) *noise.Client) Getter {
