...
import (
	...
	"wa-api/pkg/infra/noise/registry/broadcast"
	"wa-api/pkg/infra/noise/registry/clients"
	"wa-api/pkg/infra/noise/registry/userclients"
	"wa-api/pkg/infra/noise/registry/webhook"
...
// SetNoiseClient publica o cliente do SDK de userID.
func (cm *ClientManager) SetNoiseClient(userID string, client *noise.Client) {
	cm.clients.SetClient(userID, client)
}

// GetNoiseClient devolve o cliente do SDK de userID, ou nil.
func (cm *ClientManager) GetNoiseClient(userID string) *noise.Client {
	return cm.clients.GetClient(userID)
}

// DeleteNoiseClient remove o cliente do SDK de userID.
func (cm *ClientManager) DeleteNoiseClient(userID string) {
	cm.clients.DeleteClient(userID)
}

// GetAllClients devolve uma cópia do mapa de clientes do SDK.
func (cm *ClientManager) GetAllClients() map[string]*noise.Client {
	return cm.clients.Snapshot()
}

// GetNoiseClientsCount devolve quantos clientes do SDK estão registrados.
func (cm *ClientManager) GetNoiseClientsCount() int {
	return cm.clients.Count()
}

// IterateNoiseClients percorre os clientes do SDK sob lock de leitura,
// parando quando callback devolve false.
func (cm *ClientManager) IterateNoiseClients(callback func(*noise.Client) bool) {
