...
// *noise.Client do SDK que ela embrulha.
//
...
type Registry struct {
	...
	clients  map[string]*noise.Client
}
...
func New() *Registry {
	return &Registry{
		...
		clients:  make(map[string]*noise.Client),
	}
}

// Register associa a Session ao userID.
//
// Também publica o *noise.Client subjacente: os adapters de domínio (e o
...
func (r *Registry) Register(userID string, sess port.Session) {
	...
	if exposer, ok := sess.(interface{ NoiseClient() *noise.Client }); ok {
		if c := exposer.NoiseClient(); c != nil {
			r.clients[userID] = c
		}
	}
}
...
func (r *Registry) SetClient(userID string, c *noise.Client) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.clients[userID] = c
}
...
func (r *Registry) GetClient(userID string) *noise.Client {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *Registry) Snapshot() map[string]*noise.Client {
	r.mu.RLock()
	defer r.mu.RUnlock()
	out := make(map[string]*noise.Client, len(r.clients))
...
func (r *Registry) Iterate(callback func(*noise.Client) bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
