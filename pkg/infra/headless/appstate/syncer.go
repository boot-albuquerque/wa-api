...
type primer interface {
	Prime(ctx context.Context, label string) (headless.RosterPrimeResult, error)
}
...
func (s *Syncer) SyncContactRoster(ctx context.Context, txtID string, mode string) error {
	if !knownModes[mode] {
		return fmt.Errorf("headless: unknown sync mode %q", mode)
	}
...
func (s *Syncer) primer(ctx context.Context, txtID string) (primer, error) {
	...
	return headless.NewContactLister(s.sessions.Runner(), eval), nil
}
