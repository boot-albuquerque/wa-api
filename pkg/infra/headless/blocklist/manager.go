...
type blocker interface {
	Block(ctx context.Context, jid, label string) (headless.BlockResult, error)
	Unblock(ctx context.Context, jid, label string) (headless.BlockResult, error)
...
func (m *Manager) capability(ctx context.Context, txtID string) (*headless.Blocker, error) {
	eval, err := m.sessions.Evaluator(ctx, txtID)
	if err != nil {
		return nil, err
	}
	return headless.NewBlocker(m.sessions.Runner(), eval), nil
}
