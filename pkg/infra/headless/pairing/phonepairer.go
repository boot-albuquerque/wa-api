...
import (
	...
	"wa-api/internal/headless/capabilities/phonepair"
...
// already do (headless.RefreshOwnIdentity). PairPhoneUseCase calls this
...
func (p *PhonePairer) IsPaired(ctx context.Context, txtID string) (bool, error) {
	...
	identity, err := headless.RefreshOwnIdentity(ctx, p.sessions.Runner(), eval, phonePairLabel)
	if err != nil {
