...
type duplo struct {
	rename     headless.GroupRename
	descrito   headless.GroupDescribed
...
func (d *duplo) SetSubject(_ context.Context, groupJID, subject, _ string) (headless.GroupRename, error) {
	d.vistoGrupo, d.vistoTexto = groupJID, subject
	return d.rename, d.err
}

func (d *duplo) SetDescription(_ context.Context, groupJID, description, _ string) (headless.GroupDescribed, error) {
	d.vistoGrupo, d.vistoTexto = groupJID, description
	return d.descrito, d.err
}

func (d *duplo) SetPolicy(_ context.Context, groupJID string, p headless.GroupPolicy, on bool, _ string) (headless.GroupPolicyChange, error) {
...
func TestNomeSemCampoDoModeloNaoPassaPorFeito(t *testing.T) {
	d := &duplo{rename: headless.GroupRename{Field: "", AlreadyInState: false}}
...
func TestNomeJaNoEstadoNaoPrecisaDeCampo(t *testing.T) {
	d := &duplo{rename: headless.GroupRename{AlreadyInState: true}}
...
func TestDescricaoRELIDADiferenteNaoPassaPorFeita(t *testing.T) {
	d := &duplo{descrito: headless.GroupDescribed{Text: "outra coisa", Source: "desc"}}
...
func TestDescricaoSoDIFERENTENOESPACOEAceita(t *testing.T) {
	d := &duplo{descrito: headless.GroupDescribed{Text: "  combinado  ", Source: "desc"}}
...
func TestPoliticaNaoVERIFICADANaoPassaPorFeita(t *testing.T) {
	d := &duplo{mudanca: headless.GroupPolicyChange{Wanted: true, Verified: false, NoOp: false}}
...
func TestAnnounceELockedSaoPoliticasDIFERENTES(t *testing.T) {
	verificada := headless.GroupPolicyChange{Verified: true}

	d := &duplo{mudanca: verificada}
	if err := com(d).SetGroupAnnounce(context.Background(), "s1", grupo, true); err != nil {
		t.Fatalf("announce: %v", err)
	}
	if d.vistaPol != headless.PolicyMessagesAdminsOnly {
	...
	if d2.vistaPol != headless.PolicyInfoAdminsOnly {
...
func TestDesligarChegaComoDesligar(t *testing.T) {
	d := &duplo{mudanca: headless.GroupPolicyChange{Verified: true}}
...
func TestGrafiaDaPaginaChegaACapability(t *testing.T) {
	d := &duplo{rename: headless.GroupRename{Field: "subject"}}
	if err := com(d).SetGroupName(context.Background(), "s1", grupo, "equipe"); err != nil {
