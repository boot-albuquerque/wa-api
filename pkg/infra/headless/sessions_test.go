...
func cfgOK(string) (headless.StartConfig, error) {
...
func TestFalhaDeConfiguracaoPropagaComACausa(t *testing.T) {
	s := NewSessions(registry.New(1), func(string) (headless.StartConfig, error) {
...
func TestConfiguracaoERESOLVIDAANTESDeGastarSlot(t *testing.T) {
	reg := registry.New(1)
	s := NewSessions(reg, func(string) (headless.StartConfig, error) {
...
func TestRecusaPorCapacidadeChegaAoChamador(t *testing.T) {
	reg := registry.New(1)
	if _, err := reg.Acquire("ocupante", headless.StartConfig{}, registry.KindOperational); err != nil {
