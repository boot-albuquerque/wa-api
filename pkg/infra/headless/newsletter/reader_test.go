...
type followerDuplo struct {
	entries []headless.ChannelEntry
	err     error
}

func (f followerDuplo) Followed(context.Context, string) ([]headless.ChannelEntry, error) {
...
func TestOAdaptadorNaoFILTRAOQueNaoSabeJulgar(t *testing.T) {
	entradas := []headless.ChannelEntry{
...
func TestNormalizacaoDaEntradaDaPagina(t *testing.T) {
	...
	got, err := comFollower(followerDuplo{entries: []headless.ChannelEntry{{
		JID:         "1@newsletter",
		Name:        "Avisos",
