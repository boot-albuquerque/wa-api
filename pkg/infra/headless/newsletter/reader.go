...
type follower interface {
	Followed(ctx context.Context, label string) ([]headless.ChannelEntry, error)
}

type creator interface {
	Create(ctx context.Context, name, description, label string) (headless.ChannelCreated, error)
}
...
// answered `{"JID":…,"Subscribers":…,"Membership":…}` where a noise session
...
func mapChannelEntry(e headless.ChannelEntry) domain.NewsletterMetadata {
...
func (r *Reader) follower(ctx context.Context, txtID string) (follower, error) {
	...
	return headless.NewChannelManager(r.sessions.Runner(), eval), nil
}
...
func (r *Reader) CreateNewsletter(ctx context.Context, txtID, name, description string, picture []byte) (*domain.NewsletterMetadata, error) {
	...
	if len(picture) > 0 {
		return nil, fmt.Errorf("headless: criar canal COM foto esta bloqueado neste build "+
...
var errNoName = errors.New("headless: canal sem nome")
