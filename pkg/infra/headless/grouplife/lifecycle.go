...
type lifecycle interface {
	Ensure(ctx context.Context, subject string, participants []string, label string) (headless.GroupCreated, error)
	JoinByInvite(ctx context.Context, code, label string) (headless.GroupJoined, error)
...
func (m *Manager) CreateGroup(ctx context.Context, txtID, name string, participants []domain.JID, _ domain.CreateGroupOpts) (*domain.CreatedGroup, error) {
	if name == "" {
		return nil, fmt.Errorf("headless: group name is empty")
	}
