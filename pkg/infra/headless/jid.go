...
func ToPageJID(j domain.JID) (string, error) {
	...
	if raw == "" {
		return "", fmt.Errorf("headless: empty JID")
	}

	switch j.Namespace() {
	case domain.NamespacePhone:
		// Both spellings mean "phone identity"; this transport writes @c.us.
		return user(raw) + headless.ServerPhone, nil
	case domain.NamespaceLID:
		return user(raw) + headless.ServerLID, nil
	case domain.NamespaceGroup:
		return user(raw) + headless.ServerGroup, nil
	case domain.NamespaceUnknown:
		...
		if strings.Contains(raw, "@") {
			return "", fmt.Errorf("headless: %q names a namespace this transport does not serve", raw)
		}
		return raw + headless.ServerPhone, nil
	default:
		...
		return "", fmt.Errorf("headless: namespace %q has no chat form here", j.Namespace())
	}
}
