...
func TestFotoAusenteDevolveNilSemErro(t *testing.T) {
	r := comFetcher(fetcherDuplo{pic: headless.AvatarPicture{Present: false}})
...
func TestFalhaDeConfiguracaoPropaga(t *testing.T) {
	r := NewReader(adapter.NewSessions(registry.New(1), func(string) (headless.StartConfig, error) {
