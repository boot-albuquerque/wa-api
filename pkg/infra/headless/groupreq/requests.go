...
type requests interface {
	List(ctx context.Context, groupJID, label string) (headless.GroupRequestList, error)
	Approve(ctx context.Context, groupJID string, requesters []string, label string) ([]headless.GroupRequestAction, error)
	Reject(ctx context.Context, groupJID string, requesters []string, label string) ([]headless.GroupRequestAction, error)
}
...
func (m *Manager) UpdateRequestParticipants(ctx context.Context, txtID string, group domain.JID, participants []domain.JID, action domain.RequestAction) (domain.ParticipantsUpdate, error) {
	if len(participants) == 0 {
		return domain.ParticipantsUpdate{}, fmt.Errorf("headless: no requesters to %s", action)
	}
	pageJID, err := adapter.ToPageJID(group)
	if err != nil {
	...
	var results []headless.GroupRequestAction
	switch action {
	...
	default:
		return domain.ParticipantsUpdate{}, fmt.Errorf("headless: unknown request action %q", action)
	}
	if err != nil {
		return domain.ParticipantsUpdate{}, err
	}
...
func partialFailure(results []headless.GroupRequestAction, asked int, action domain.RequestAction) error {
	failed := 0
	...
	return fmt.Errorf("headless: %s incompleto: %d de %d recusados pela pagina (codigos %v), %d sem resposta",
...
func (m *Manager) SetJoinApprovalMode(ctx context.Context, txtID string, group domain.JID, mode bool) error {
	...
	change, err := p.SetPolicy(ctx, pageJID, headless.PolicyJoinNeedsApproval, mode, approvalLabel)
	if err != nil {
		return err
	}
...
func unverified(change headless.GroupPolicyChange) error {
	if change.NoOp || change.Verified {
		return nil
	}
	return fmt.Errorf("headless: modo de aprovacao pedido (%t) sem confirmacao lida de volta", change.Wanted)
}
...
func (m *Manager) requests(ctx context.Context, txtID string) (requests, error) {
	...
	return headless.NewGroupRequestManager(m.sessions.Runner(), eval), nil
}
