...
type duploPedidos struct {
	lista       headless.GroupRequestList
	resultados  []headless.GroupRequestAction
	vistoGrupo  string
	vistoQuem   []string
	vistoRejeic bool
	err         error
...
func (d *duploPedidos) List(_ context.Context, groupJID, _ string) (headless.GroupRequestList, error) {
	d.vistoGrupo = groupJID
	return d.lista, d.err
}

func (d *duploPedidos) Approve(_ context.Context, groupJID string, quem []string, _ string) ([]headless.GroupRequestAction, error) {
	d.vistoGrupo, d.vistoQuem = groupJID, quem
	return d.resultados, d.err
}

func (d *duploPedidos) Reject(_ context.Context, groupJID string, quem []string, _ string) ([]headless.GroupRequestAction, error) {
	d.vistoGrupo, d.vistoQuem, d.vistoRejeic = groupJID, quem, true
	return d.resultados, d.err
}

type duploPolitica struct {
	mudanca    headless.GroupPolicyChange
	vistoGrupo string
	vistaPol   headless.GroupPolicy
	vistoOn    bool
	err        error
}

func (d *duploPolitica) SetPolicy(_ context.Context, groupJID string, p headless.GroupPolicy, on bool, _ string) (headless.GroupPolicyChange, error) {
...
func ok(jid string) headless.GroupRequestAction {
	return headless.GroupRequestAction{RequesterJID: jid, OK: true}
}

func recusado(jid string, code int) headless.GroupRequestAction {
	return headless.GroupRequestAction{RequesterJID: jid, OK: false, Code: code}
}
...
func TestRecusaParcialNaoPassaPorSucesso(t *testing.T) {
	d := &duploPedidos{resultados: []headless.GroupRequestAction{
...
func TestSolicitanteSemRespostaNaoContaComoAprovado(t *testing.T) {
	d := &duploPedidos{resultados: []headless.GroupRequestAction{ok("a@lid")}}
	_, err := com(d, nil).UpdateRequestParticipants(context.Background(), "s1", grupo,
...
func TestTodosAprovadosNaoEErro(t *testing.T) {
	d := &duploPedidos{resultados: []headless.GroupRequestAction{ok("a@lid"), ok("b@lid")}}
...
func TestGrafiaDaPaginaChegaAsCapabilities(t *testing.T) {
	...
	p := &duploPolitica{mudanca: headless.GroupPolicyChange{Wanted: true, Verified: true}}
...
func TestOModoDeAprovacaoEAPoliticaCerta(t *testing.T) {
	p := &duploPolitica{mudanca: headless.GroupPolicyChange{Wanted: false, Verified: true}}
	if err := com(nil, p).SetJoinApprovalMode(context.Background(), "s1", grupo, false); err != nil {
		t.Fatalf("SetJoinApprovalMode: %v", err)
	}
	if p.vistaPol != headless.PolicyJoinNeedsApproval {
...
func TestMudancaNaoVERIFICADANaoPassaPorFeita(t *testing.T) {
	p := &duploPolitica{mudanca: headless.GroupPolicyChange{Wanted: true, Verified: false, NoOp: false}}
	if err := com(nil, p).SetJoinApprovalMode(context.Background(), "s1", grupo, true); err == nil {
...
func TestNoOpNaoPrecisaDeConfirmacao(t *testing.T) {
	p := &duploPolitica{mudanca: headless.GroupPolicyChange{Wanted: true, NoOp: true, Verified: false}}
	if err := com(nil, p).SetJoinApprovalMode(context.Background(), "s1", grupo, true); err != nil {
