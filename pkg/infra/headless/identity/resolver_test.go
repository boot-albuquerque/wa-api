...
func TestFalhaDeLookupABORTAEmVezDeVirarAusencia(t *testing.T) {
	naoEsta := fmt.Errorf("envelope: %w", headless.ErrNotOnWhatsApp)
	if check, err := classifyCheck("5511999999999", headless.Identity{}, naoEsta); err != nil {
	...
	if _, err := classifyCheck("5511999999999", headless.Identity{}, falhaReal); err == nil {
	...
	if check, err := classifyCheck("5511999999999", headless.Identity{JID: "x@lid"}, nil); err != nil || !check.IsIn {
...
type lookuperDuplo struct {
	pares map[string]headless.IdentityPair
	err   error
}

func (l lookuperDuplo) NumberID(context.Context, string, string) (headless.Identity, error) {
	return headless.Identity{}, l.err
}

func (l lookuperDuplo) LidAndPhone(_ context.Context, jid, _ string) (headless.IdentityPair, error) {
	if l.err != nil {
		return headless.IdentityPair{}, l.err
	}
...
func TestMapeamentoAusenteEJIDVaziaSemErro(t *testing.T) {
	r := comLookuper(lookuperDuplo{pares: map[string]headless.IdentityPair{}})
...
func TestAsDuasDirecoesNaoSaoTrocadas(t *testing.T) {
	...
	l := lookuperDuplo{pares: map[string]headless.IdentityPair{
...
func TestFalhaDeConfiguracaoPropaga(t *testing.T) {
	r := NewResolver(adapter.NewSessions(registry.New(1), func(string) (headless.StartConfig, error) {
