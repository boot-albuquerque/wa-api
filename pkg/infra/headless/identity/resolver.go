...
type lookuper interface {
	NumberID(ctx context.Context, jid, label string) (headless.Identity, error)
	LidAndPhone(ctx context.Context, jid, label string) (headless.IdentityPair, error)
}
...
// suite stayed green.
func classifyCheck(phone string, id headless.Identity, err error) (domain.WhatsAppCheck, error) {
	switch {
	case errors.Is(err, headless.ErrNotOnWhatsApp):
...
func (r *Resolver) pair(ctx context.Context, txtID string, j domain.JID) (headless.IdentityPair, error) {
	pageJID, err := adapter.ToPageJID(j)
	if err != nil {
		return headless.IdentityPair{}, err
	}
	resolver, err := r.lookuper(ctx, txtID)
	if err != nil {
		return headless.IdentityPair{}, err
...
func (r *Resolver) capability(ctx context.Context, txtID string) (*headless.Resolver, error) {
	eval, err := r.sessions.Evaluator(ctx, txtID)
	if err != nil {
		return nil, err
	}
	return headless.NewResolver(r.sessions.Runner(), eval), nil
}
