...
var chatStates = map[string]headless.PresenceState{
	string(headless.PresenceComposing): headless.PresenceComposing,
	string(headless.PresencePaused):    headless.PresencePaused,
	string(headless.PresenceRecording): headless.PresenceRecording,
	// The socket transport's vocabulary for the same three, so a caller written
	// against it keeps working.
	"typing":  headless.PresenceComposing,
	"stopped": headless.PresencePaused,
	"pause":   headless.PresencePaused,
	"audio":   headless.PresenceRecording,
}

// announcer is the slice of the page capability this adapter uses.
type announcer interface {
	Set(ctx context.Context, toJID string, s headless.PresenceState, label string) error
...
func (a *Announcer) SendPresence(ctx context.Context, txtID string, p domain.PresenceType) error {
	...
	case domain.PresenceUnavailable:
		available = false
	default:
		return fmt.Errorf("headless: unknown presence %q", p)
	}
...
func (a *Announcer) SendChatPresence(ctx context.Context, txtID string, chat domain.JID, state, _ string) error {
	...
	if !ok {
		return fmt.Errorf("%w: %q", headless.ErrUnknownPresenceState, state)
	}
...
func (a *Announcer) capability(ctx context.Context, txtID string) (*headless.PresenceAnnouncer, error) {
	eval, err := a.sessions.Evaluator(ctx, txtID)
	if err != nil {
		return nil, err
	}
	return headless.NewPresence(a.sessions.Runner(), eval), nil
}
