...
func TestEstadoDeConversaEMapeadoContraConjuntoFechado(t *testing.T) {
	...
	if !errors.Is(err, headless.ErrUnknownPresenceState) {
...
type announcerDuplo struct {
	jid        string
	estado     headless.PresenceState
	disponivel bool
	online     bool
}

func (a *announcerDuplo) Set(_ context.Context, jid string, s headless.PresenceState, _ string) error {
...
func TestOVocabularioDoSocketETraduzidoENaoRepassado(t *testing.T) {
	...
	if d.estado != headless.PresenceComposing {
		t.Fatalf("estado=%q, quero %q: o vocabulário do socket não foi traduzido",
			d.estado, headless.PresenceComposing)
	}
}
...
func TestFalhaDeConfiguracaoPropaga(t *testing.T) {
	a := NewAnnouncer(adapter.NewSessions(registry.New(1), func(string) (headless.StartConfig, error) {
