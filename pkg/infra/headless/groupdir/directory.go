...
type lister interface {
	List(ctx context.Context, limit int, label string) (headless.ChatList, error)
	ByJID(ctx context.Context, jid, label string) (headless.Chat, error)
}

type inviter interface {
	InviteCode(ctx context.Context, groupJID, label string) (headless.GroupInviteCode, error)
	InviteInfo(ctx context.Context, code, label string) (headless.GroupInvite, error)
}
...
func (d *Directory) GetGroupInfo(ctx context.Context, txtID string, groupJID domain.JID) (*domain.GroupInfo, error) {
	...
	if !chat.IsGroup {
		return nil, fmt.Errorf("headless: %q is not a group conversation", pageJID)
	}
...
func toDomainGroupInfo(chat headless.Chat) *domain.GroupInfo {
	return &domain.GroupInfo{
		...
		// propriedade que o noise chama IsAnnounce.
		IsAnnounce: chat.ReadOnly,
		// Não-nulo mesmo vazio, pela mesma razão do adaptador noise: o
...
func (d *Directory) GetGroupInfoFromLink(ctx context.Context, txtID, code string) (*domain.GroupInfo, error) {
	if code == "" {
		return nil, fmt.Errorf("headless: empty invite code")
	}
	inv, err := d.inviter(ctx, txtID)
	if err != nil {
		return nil, err
...
func (d *Directory) grupos(ctx context.Context, txtID string) ([]headless.Chat, error) {
	l, err := d.lister(ctx, txtID)
	if err != nil {
		return nil, err
	}
	...
	out := make([]headless.Chat, 0, len(lista.Chats))
...
func (d *Directory) lister(ctx context.Context, txtID string) (lister, error) {
	...
	return headless.NewChatLister(d.sessions.Runner(), eval), nil
}
...
func (d *Directory) inviter(ctx context.Context, txtID string) (inviter, error) {
	...
	return headless.NewGroupManager(d.sessions.Runner(), eval), nil
}
