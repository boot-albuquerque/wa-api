...
func (s *setterDuplo) SetArchived(_ context.Context, jid string, archived bool, _ string) (headless.ChatStateChange, error) {
	s.jid, s.archived = jid, archived
	return headless.ChatStateChange{}, s.err
}
...
func TestOJIDChegaConvertidoACapability(t *testing.T) {
	...
	if !strings.HasSuffix(d.jid, headless.ServerPhone) {
		t.Fatalf("a capability recebeu %q, quero sufixo %q", d.jid, headless.ServerPhone)
	}
}
...
func TestFalhaDeConfiguracaoPropaga(t *testing.T) {
	a := NewArchiver(adapter.NewSessions(registry.New(1), func(string) (headless.StartConfig, error) {
