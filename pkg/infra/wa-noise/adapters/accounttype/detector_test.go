...
func TestDetect_NoSession(t *testing.T) {
	d := NewDetector(func(string) waclient.Client { return nil })
...
// TestDetect_UnsupportedClientType: um waclient.Client que nao envolve
// *wanoise.Client (o duble de teste, aqui) nunca deve virar personal por
// omissao — o adaptador nao tem como medir nada dele.
func TestDetect_UnsupportedClientType(t *testing.T) {
	fake := &testkit.Fake{}
	d := NewDetector(func(string) waclient.Client { return fake })

	kind, err := d.Detect(context.Background(), "user-1")
	if err == nil {
		t.Fatal("expected error for a client that is not waclient.RealClient")
	}
