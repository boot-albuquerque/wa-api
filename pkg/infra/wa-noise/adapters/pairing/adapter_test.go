...
import (
	...
	pairingadapter "wa-api/pkg/infra/wa-noise/adapters/pairing"
...
func adapterFor(f *testkit.Fake) *pairingadapter.PhonePairerAdapter {
	return pairingadapter.NewPhonePairerAdapter(testkit.GetterWith(map[string]waclient.Client{txtID: f}))
}
...
func TestPhonePairer_SemSessao(t *testing.T) {
	a := pairingadapter.NewPhonePairerAdapter(func(string) waclient.Client { return nil })
