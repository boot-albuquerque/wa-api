...
func (a *ChatMessengerAdapter) SendCarousel(ctx context.Context, txtID string, target domain.JID, payload domain.CarouselPayload, id string) (domain.MessageSendResult, error) {
...
func (a *ChatMessengerAdapter) buildCarouselCard(ctx context.Context, client waclient.Client, card domain.CarouselCard) (*waE2E.InteractiveMessage, error) {
	...
	if len(card.Image) > 0 {
		uploaded, err := client.Upload(ctx, card.Image, wanoise.MediaImage)
		if err != nil {
			return nil, err
		}
