...
func mutationGetter(f *testkit.Fake) waclient.Getter {
	return testkit.GetterWith(map[string]waclient.Client{"u1": f})
}
...
func TestChatMessengerAdapter_RevokeMessage_SenderIsEmptyJID(t *testing.T) {
	...
	f := &testkit.Fake{
		...
		SendMessageFn: func(context.Context, types.JID, *waE2E.Message, ...wanoise.SendRequestExtra) (wanoise.SendResponse, error) {
			return wanoise.SendResponse{ID: "revoke-wire-id", Timestamp: time.Unix(1755500110, 0)}, nil
		},
	}
...
func TestChatMessengerAdapter_EditMessage_WireShape(t *testing.T) {
	...
	f := &testkit.Fake{
		SendMessageFn: func(_ context.Context, _ types.JID, msg *waE2E.Message, _ ...wanoise.SendRequestExtra) (wanoise.SendResponse, error) {
			sent = msg
			return wanoise.SendResponse{ID: "edit-wire-id", Timestamp: time.Unix(1755500111, 0)}, nil
		},
	}
	a := NewChatMessengerAdapter(mutationGetter(f))
...
func TestChatMessengerAdapter_Mutation_SendFailurePropagates(t *testing.T) {
	...
	f := &testkit.Fake{
		SendMessageFn: func(context.Context, types.JID, *waE2E.Message, ...wanoise.SendRequestExtra) (wanoise.SendResponse, error) {
			return wanoise.SendResponse{}, sentinel
		},
	}
	a := NewChatMessengerAdapter(mutationGetter(f))
