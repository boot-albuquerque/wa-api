...
func TestNewMiscAdapter(t *testing.T) {
	if NewMiscAdapter(waclienttest.GetterWith(nil)) == nil {
...
func TestMiscAdapter_ArchiveChat_OK(t *testing.T) {
	called := false
	fake := &waclienttest.Fake{SendAppStateFn: func(ctx context.Context, patch appstate.PatchInfo) error {
		called = true
		return nil
	}}
...
func TestMiscAdapter_ArchiveChat_PropagatesError(t *testing.T) {
	...
	fake := &waclienttest.Fake{SendAppStateFn: func(ctx context.Context, patch appstate.PatchInfo) error { return sdkErr }}
	a := NewMiscAdapter(waclienttest.GetterWith(map[string]waclient.Client{"u1": fake}))
...
func TestMiscAdapter_RejectCall_OK(t *testing.T) {
	called := false
	fake := &waclienttest.Fake{RejectCallFn: func(ctx context.Context, callFrom types.JID, callID string) error {
		called = true
		if callID != "call-1" {
...
func TestMiscAdapter_SyncContactRoster_ModeMapping(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.mode, func(t *testing.T) {
			...
			called := false
			fake := &waclienttest.Fake{FetchAppStateFn: func(ctx context.Context, name appstate.WAPatchName, fullSync, onlyIfNotSynced bool) error {
				called = true
				gotName = name
