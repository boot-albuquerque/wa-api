...
//
// Implementa WaNoiseClient() devolvendo um ponteiro nao-nil para que
// Register exercite de fato a escrita nos DOIS mapas (sessions e
// wanoiseClients); devolvendo nil, o `if client != nil` de Register
// pularia a segunda escrita e o teste nao cobriria o caminho que a quebra
// em sub-registries precisa preservar.
type raceSession struct{ client *wanoise.Client }

func (s *raceSession) WaNoiseClient() *wanoise.Client { return s.client }
...
var inertSession = &raceSession{client: &wanoise.Client{}}
...
func TestClientManagerAcessoConcorrenteNaoTemCorrida(t *testing.T) {
	...
	// wanoiseClients — escrita, leitura, remocao, snapshot, contagem e
	// iteracao, todos concorrentes entre si.
	run(func(w, i int) { cm.SetWaNoiseClient(uid(w, i), nil) })
	run(func(w, i int) { _ = cm.GetWaNoiseClient(uid(w, i)) })
	run(func(w, i int) { cm.DeleteWaNoiseClient(uid(w, i)) })
	...
	// sessions — Register escreve sessions E wanoiseClients.
	...
	cm.SetWaNoiseClient("depois", nil)
