...
type inertSession struct{ client *wanoise.Client }

func (s *inertSession) WaNoiseClient() *wanoise.Client { return s.client }
...
func TestCountEIterate(t *testing.T) {
	r := New()
	a, b := &wanoise.Client{}, &wanoise.Client{}
	r.SetClient("a", a)
	r.SetClient("b", b)
	...
	visitados := 0
	r.Iterate(func(*wanoise.Client) bool { visitados++; return true })
	if visitados != 2 {
	...
	visitados = 0
	r.Iterate(func(*wanoise.Client) bool { visitados++; return false })
	if visitados != 1 {
...
func TestRegistryConcorrente(t *testing.T) {
	...
	sess := &inertSession{client: &wanoise.Client{}}
	...
	run(func(w, i int) { r.Iterate(func(*wanoise.Client) bool { return true }) })
