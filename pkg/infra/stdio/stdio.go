// Package stdio provides a JSON-RPC 2.0 server over stdin/stdout.
//
...
// ServerHandler is the interface that the stdio server needs from the main server
type ServerHandler interface {
	ServeHTTP(w http.ResponseWriter, r *http.Request)
	SendNotificationToStdio(method string, params map[string]interface{})
}

// Server handles stdin/stdout JSON-based API by wrapping HTTP handlers
type Server struct {
	router interface{} // Use interface{} to hold the router
	stdin  io.Reader
	stdout io.Writer
}

// NewServer creates a new stdio server instance
func NewServer(router interface{}) *Server {
	return &Server{
		router: router,
		stdin:  os.Stdin,
		stdout: os.Stdout,
...
// NewServerWithIO creates a stdio server with custom IO streams (for testing)
func NewServerWithIO(router interface{}, stdin io.Reader, stdout io.Writer) *Server {
	return &Server{
		router: router,
		stdin:  stdin,
		stdout: stdout,
...
func (ss *Server) executeHTTPHandler(req *JSONRpcRequest, httpMethod, httpPath string) {
	...
	if handler, ok := ss.router.(http.Handler); ok {
...
func (ss *Server) convertHTTPResponse(requestID ID, recorder *httptest.ResponseRecorder) {
...
func (ss *Server) sendSuccess(id ID, code int, data interface{}) {
...
func (ss *Server) sendError(id ID, code int, errorMsg string) {
	response := JSONRpcResponse{
		JSONRPC: "2.0",
		ID:      id,
		Error: &RpcError{
...
func (ss *Server) writeResponse(response JSONRpcResponse) {
...
func SendNotification(method string, params map[string]interface{}) {
