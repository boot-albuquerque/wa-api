...
const (
	...
	// empty, or not one of the two values domain.EngineValido accepts
	// (pkg/domain/user.go) — unlike TargetEngine's own read of a persisted
	// row, an empty value here is never defaulted.
...
// errInvalidEngineValue is the wrapped cause for a request whose engine is
// missing, empty, or not one of the two this contract accepts.
var errInvalidEngineValue = fmt.Errorf("invalid engine: must be %q or %q", domain.EngineNoise, domain.EngineWaHeadless)
...
func errInvalidEngine(raw string) error {
	return apperr.New(CodeInvalidEngine, apperr.CategoryValidation,
		fmt.Sprintf("engine must be %q or %q", domain.EngineNoise, domain.EngineWaHeadless),
		false, fmt.Errorf("%w (got %q)", errInvalidEngineValue, raw))
}
...
func errEngineMismatch(requested, target string) error {
...
func errCapabilityNotSupported(capability domain.Capability, engine string, status domain.CapabilityStatus, reason string) error {
...
func errEngineUnavailable(capability domain.Capability, engine string) error {
