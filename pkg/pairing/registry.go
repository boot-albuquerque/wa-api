...
func NewRegistry(sessions SessionEngineReader, caps *capabilityregistry.CapabilityRegistry, providers ...*Provider) *Registry {
	byEngine := make(map[string]*Provider, len(providers))
...
//
// A row whose engine is empty or not one of the two known values is reported
// as domain.EngineNoise, via domain.EngineValido's own default — migration
// 19 gives the column NOT NULL DEFAULT 'noise', so this is a defensive
// fallback, not a state this process expects to observe in practice.
func (r *Registry) TargetEngine(ctx context.Context, targetSessionID string) (string, error) {
	...
	engine, ok := domain.EngineValido(entries[0].Engine, domain.EngineNoise)
	if !ok {
...
func (r *Registry) Resolve(ctx context.Context, targetSessionID, requestedEngine string, capability domain.Capability) (*Provider, error) {
	if requestedEngine != domain.EngineNoise && requestedEngine != domain.EngineWaHeadless {
		log.Warn().Str("target_session_id", targetSessionID).
	...
	if engine != targetEngine {
		log.Warn().Str("target_session_id", targetSessionID).
			Str("requested_engine", engine).
			Str("target_engine", targetEngine).
			Str("capability", capability.String()).
	...
	if err != nil {
		log.Error().Err(err).Str("engine", engine).Str("capability", capability.String()).
	...
	if !decision.Supported {
		log.Warn().Str("target_session_id", targetSessionID).
			Str("engine", engine).Str("capability", capability.String()).
	...
	if !ok || provider == nil {
		log.Error().Str("engine", engine).Str("capability", capability.String()).
...
func (r *Registry) ResolveQRReader(ctx context.Context, targetSessionID, requestedEngine string) (appport.PairingQRReader, error) {
	...
	if provider.QRReader == nil {
		log.Error().Str("engine", provider.Engine).
