...
func sessionCaseRegistry(waNoise *pairing.Provider) *pairing.Registry {
	users := &contractsfake.UserRepository{
		ListUsersFunc: func(_ context.Context, id string) ([]domain.UserListEntry, error) {
			...
			return []domain.UserListEntry{{ID: id, Engine: domain.EngineNoise, QRCode: "qr-data"}}, nil
		},
	}
...
func TestGetQRAndStatus_NoUserRecord_400_NoSession(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			name:    "GetQR",
			handler: NewGetQRHandler(log, pairing.NewRegistry(empty, capabilityregistry.NewCapabilityRegistry(), &pairing.Provider{Engine: domain.EngineNoise, QRReader: &contractsfake.PairingQRReader{}}, &pairing.Provider{Engine: domain.EngineWaHeadless})),
			path:    "/session/qr?engine=noise",
		},
		{
			name:    "GetStatus",
...
func TestGetQRAndStatus_RepositoryFailure_500_LogsError(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			name:    "GetQR",
			handler: NewGetQRHandler(log, pairing.NewRegistry(broken, capabilityregistry.NewCapabilityRegistry(), &pairing.Provider{Engine: domain.EngineNoise, QRReader: &contractsfake.PairingQRReader{}}, &pairing.Provider{Engine: domain.EngineWaHeadless})),
			path:    "/session/qr?engine=noise",
		},
		{
			name:    "GetStatus",
