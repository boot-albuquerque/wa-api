...
// TestLiveNaming_Groups covers the groups family via POST /group/info — the
// REFERENCE route (docs/HTTP-DTO-CONVENTIONS.md's implementação de
// referência) already migrated to the DTO/presenter pattern.
// handler_group_info_contract_test.go asserts this same route in more
...
// GET /newsletter/list. The port's ListSubscribed returns `any` "sem
// transformação" (pkg/application/contracts/misc_ports.go) — the adapter
// (pkg/infra/wa-noise/adapters/misc/adapter.go) hands back
// []types.NewsletterMetadata verbatim, so the fixture uses the REAL
// vendored type instead of a map: types.JID has no custom MarshalJSON, so
// its nested User/Server fields serialize as PascalCase regardless of what
// this test's fixture looks like — that's a property of the library type,
// worth measuring rather than papering over with a fabricated map.
	...
	nr := &contractsfake.NewsletterReader{
		ListSubscribedFunc: func(context.Context, string) (any, error) {
			return []types.NewsletterMetadata{{
				ID: types.JID{User: "120363000000000000", Server: "newsletter"},
				ThreadMeta: types.NewsletterThreadMetadata{
					Name: types.NewsletterText{Text: "Canal X"},
				},
			}}, nil
		},
	}
