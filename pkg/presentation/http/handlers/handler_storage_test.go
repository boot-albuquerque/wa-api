...
// TestStorageHandlers_SessionFailure_500: a sessao do noise falta. E' falha
...
func TestStorageHandlers_SessionFailure_500(t *testing.T) {
	sessionErr := errors.New("no noise session for user")
	for _, tc := range storageCases() {
		t.Run(tc.name, func(t *testing.T) {
...
func TestStorageHandlers_PayloadSecretsNeverReachTheLog(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// pode vazar.
			sg := storageSession(errors.New("no noise session for user"))
