...
func (h *SendEditMessageHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if !ok || info == nil {
		hlog.FromRequest(r).Warn().Err(errUnauthorized).
			Str("path", r.URL.Path).
			Msg("edit message rejected at the auth boundary")
	...
	if txtID == "" {
		hlog.FromRequest(r).Warn().Err(errMissingSessionID).
			Str("path", r.URL.Path).
			Msg("edit message rejected without session id")
		customhttp.RespondJSON(w, http.StatusBadRequest, nil, errMissingSessionID)
		return
	}

	var req dtomessage.SendEditMessageRequest
	if err := decodeRequest(w, r, &req); err != nil {
		...
		hlog.FromRequest(r).Warn().Err(err).
			Str("path", r.URL.Path).
			Str("user_id", txtID).
			Msg("edit message payload could not be decoded")
	...
	if err != nil {
		hlog.FromRequest(r).Error().Err(err).
			Str("path", r.URL.Path).
			Str("user_id", txtID).
			Msg("edit message use case failed")
