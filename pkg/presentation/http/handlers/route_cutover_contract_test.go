...
func TestGetGroupInviteInfo_CaminhoDoInviteCodeChegaAoUseCase(t *testing.T) {
	...
	f := newGrpFakes()
	var recebeu string
	f.directory.GetGroupInfoFromLinkFunc = func(_ context.Context, _ string, c string) (*domain.GroupInfo, error) {
		recebeu = c
		return &domain.GroupInfo{JID: "120363411669320145@g.us"}, nil
	}
...
func TestSendPollVote_CaminhoDoPollMessageIDChegaAoUseCase(t *testing.T) {
	...
	// Corpo sem poll_message_id: os outros três campos obrigatórios vêm no
	// corpo, o identificador da enquete tem de vir do caminho.
	body := `{"phone":"120363411669320145@g.us","sender":"5511999999999@s.whatsapp.net","poll_message_timestamp":1755500100,"options":["12h"]}`
