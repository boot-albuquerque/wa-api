...
| `sendReaction` | capabilities/react | `PARTIAL` | sim | sim | sim | H53: adicionar provado; remover devolve Verified:false. **H154: a causa disso caiu** — a fonte para "quais reações" existe (`Reactions.find` pelo objeto id) e `message.ReactionsOf` a lê. Verificar o `Remove` passou a ser trabalho ACIONÁVEL, não impedimento; fica registrado como tal em vez de a linha ser fechada por associação |
