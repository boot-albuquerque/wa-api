...
// HTTPClientManager interface for getting per-user HTTP clients
type HTTPClientManager interface {
	GetHTTPClient(userID string) *resty.Client
}

// CallHook sends a webhook for regular messages.
func CallHook(
	myurl string,
...
// CallHookWithHmac sends a webhook for regular messages with HMAC signature.
func CallHookWithHmac(
	myurl string,
	payload map[string]string,
	userID string,
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		if format == "json" {
			...
			if len(encryptedHmacKey) > 0 && len(jsonBody) > 0 {
				var err error
				hmacSignature, err = GenerateHmacSignature(jsonBody, encryptedHmacKey, encryptionKey)
				if err != nil {
			...
			if len(encryptedHmacKey) > 0 {
				...
				var err error
				hmacSignature, err = GenerateHmacSignature([]byte(formString), encryptedHmacKey, encryptionKey)
				if err != nil {
					log.Error().Err(err).Msg("Failed to generate HMAC signature")
				}
			}
	...
	if lastError != nil {
		...
		errorPayload := WebhookErrorPayload{
			...
			UserID:           userID,
			EncryptedHmacKey: fmt.Sprintf("%x", encryptedHmacKey),
...
// CallHookFile sends a webhook for messages with file attachments.
func CallHookFile(
	myurl string,
	payload map[string]string,
	userID string,
...
// CallHookFileWithHmac sends a webhook for messages with file attachments and HMAC signature.
func CallHookFileWithHmac(
	myurl string,
	payload map[string]string,
	userID string,
	file string,
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		if len(encryptedHmacKey) > 0 {
			...
			if err != nil {
				...
				hmacSignature, err = GenerateHmacSignature(jsonPayload, encryptedHmacKey, encryptionKey)
				if err != nil {
					log.Error().Err(err).Msg("Failed to generate HMAC signature")
				}
			}
