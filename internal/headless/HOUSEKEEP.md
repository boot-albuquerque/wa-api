# HOUSEKEEP — headless (pilha de automação de browser)

Achados incidentais de `internal/headless/`, a pilha que conduz o SPA real
do `web.whatsapp.com` dentro de um browser. É a segunda via de transporte do
projeto, ao lado de `internal/noise/`, que fala o protocolo direto no
WebSocket com o Noise framework.

Achados das outras duas fronteiras moram noutros arquivos:

- `internal/noise/HOUSEKEEP.md` — a pilha de protocolo;
...
usada pela raiz e pelo `noise`. Os dois prefixos convivem no repositório sem
...
`internal/headless/observability/`.

**Onde**: `internal/headless/observability/oplog.go`, campo `OpRecord.Err` e
...
`HEADLESS_OP_TRACE`; (b) redator injetável no `OpLog`, com o padrão sendo
...
**Escape hatch mantido**: sob `HEADLESS_OP_TRACE` o texto completo sobrevive.
...
**Data**: 2026-08-11 · **Contexto**: CAP-02, commit `headless: add target
priming`.
...
para o CAP-02 — "nada consome `internal/headless` hoje". A promessa não
...
`docs/architecture/headless-chromium-cdp.md` §2.1.1 e
`docs/adr/0038-headless-absorve-wa-worker-servico-go-unico.md`.
...
**Onde**: `internal/headless/engine/profile.go`, `readLockHolder`.
...
`internal/headless/realspa_test.go`. Sobe o perfil com o **`engine.Launcher`
...
`TestRealSPASingletonLockFormat` (gated por `HEADLESS_REAL_SPA`) e pelos
...
**Onde**: `internal/headless/integration_test.go:197-202`, o `defer` de
...
esta sessão. O `go test -race ./internal/headless/...` rodado hoje **não**
...
**Onde**: `internal/headless/spa/probe.go:83-87` e `:116-123`.
...
go test -race -count=1 ./internal/headless/...
  wa-api/internal/headless               ok  98.166s
  wa-api/internal/headless/engine        ok   5.015s
  wa-api/internal/headless/observability ok   1.189s
  wa-api/internal/headless/spa           ok   2.222s
make lint   269 issues  ·  pai (stash, MESMA árvore) 269  ·  delta 0
  internal/headless: as MESMAS 2 gocyclo pré-existentes, nenhuma tocada
```
...
**Onde**: `internal/headless/spa/probe.go`, `structureScript`
(`title: document.title`) e `internal/headless/spa/page.go`,
`PageSnapshot.Title`.
...
1. **Ninguém lê o campo.** `grep -rn '\.Title\b' --include='*.go'
   internal/headless/` não devolve **nenhum** leitor. Ele é decodificado do
...
**Onde**: `internal/headless/spa/page.go`, no `Classify`:
...
- toda parada suja escreve `.headless-session-suspect` **dentro do perfil**,
...
**Onde**: `internal/headless/EVIDENCIA-SPA.md`, seção **M6.2**, cabeçalho da
...
**Onde**: `internal/headless/engine/network_test.go`, o antigo
...
cópia: `ok wa-api/internal/headless/engine 3.783s`.
...
FAIL
FAIL	wa-api/internal/headless/engine	0.198s
```
...
ok  	wa-api/internal/headless/engine	0.191s
```
...
**Onde**: `internal/headless/EXECUCAO.md:904-910`, dentro do F-24:
...
independente de `internal/headless/spa/socket.go`, arquivo novo que
introduz o limiar `C = 3030ms`.

**Onde**: `internal/headless/spa/socket.go` (comentário de derivação de
`OpeningWindowThreshold`) e `internal/headless/spa/socket_test.go`
...
**Data**: 2026-08-18 · **Contexto**: LOOP 04.4, avaliação adversarial
independente de `internal/headless/spa/socket.go`, mesma revisão da H14.

**Onde**: `internal/headless/spa/socket.go`, comentário de
...
**Onde**: `internal/headless/spa/socket_test.go`,
...
**Onde**: `internal/headless/spa/socket.go:100`.
...
palpite: `grep` por `Socket|Liveness|ClassifyOpening` em `internal/headless/core/`
...
**Onde**: `internal/headless/core/session.go`, tipo `BootFailure`.
...
**Onde**: `internal/headless/core/session.go` — `engine.SessionSuspect` na
...
**Correção sugerida**: fixture que pré-grava `.headless-session-suspect` e
...
**Onde**: `internal/headless/core/` — em especial
`TestStartSession_SuspectMarker_NotClearedOnFailedBoot` e os testes cuja página
nunca monta.

**Problema**: `go test -race -count=1 ./internal/headless/core/...` foi de
...
**Onde**: API pública de `internal/headless/core` — `Session` expõe
...
**Onde**: `internal/headless/realspa_test.go`, commit `4969834`.
...
**Onde**: `internal/headless/engine/browser.go`, `Browser.PID()`.
...
**Onde**: `internal/headless/capabilities/messagemeta/messagemeta.go`,
constante `eventAdd`.
...
`HEADLESS_MSG_DELIVERY`, observa por 3 minutos enquanto um humano envia uma
...
**Onde**: `internal/headless/core/session_test.go`,
...
**Onde**: `internal/headless/capabilities/backup/backup.go`, `copyFile`.
...
**Testes que travam** (`internal/headless/`):
...
**Onde**: `internal/headless/integration_test.go:389-405`. As três chamadas
...
**Onde ficou**: `internal/headless/probe_modmap_test.go`, guardada por
...
**Onde**: `internal/headless/capabilities/send/send.go`, no `dispatchScript`.
...
**Onde ficou**: `internal/headless/capabilities/contacts/`, com a fusão em GO
...
**Onde**: `internal/headless/engine/runner.go:74-113`.
...
(`CC.trigger('headlessSelfTest')`) e a verificar que o handler o viu. Viu:
...
**Onde**: `internal/headless/capabilities/messagemeta/messagemeta.go`, no
...
encontrar: `headless-lab — teste automatizado, pode apagar`.
...
**Onde**: `internal/headless/capabilities/group/participants.go`,
`internal/headless/spa/modules.go` (`ModuleGroupParticipantsJob`).
...
**Onde**: `internal/headless/capabilities/block/`,
`internal/headless/spa/modules_block.go`,
`internal/headless/probe_block_test.go`.
...
**Onde**: `internal/headless/capabilities/edit/`,
`internal/headless/spa/modules_block.go`,
`internal/headless/probe_edit_test.go`.
...
**Onde**: `internal/headless/capabilities/star/`,
`internal/headless/spa/modules_block.go`,
`internal/headless/probe_star_test.go`.
...
**Data**: 2026-08-21
**Contexto**: continuação da superfície do whatsapp-web.js.
**Onde**: `internal/headless/capabilities/mute/`,
`internal/headless/spa/modules_block.go`,
`internal/headless/probe_mute_test.go`.
...
**Onde**: `internal/headless/capabilities/forward/`,
`internal/headless/spa/modules_block.go`,
`internal/headless/probe_forward_test.go`.
...
**Onde**: `internal/headless/capabilities/group/subject.go`,
`internal/headless/spa/modules_block.go`,
`internal/headless/utf16len_test.go`.
...
**Onde**: `internal/headless/capabilities/group/admin.go`.
...
`internal/headless/capabilities/profile/`.
...
**Data**: 2026-08-21
**Contexto**: Fase 1, cobertura.
**Onde**: `internal/headless/capabilities/media/download.go`.
...
**Onde**: `internal/headless/capabilities/contacts/commongroups.go`.
...
**Data**: 2026-08-21
**Contexto**: Fase 1, cobertura.
**Onde**: `internal/headless/capabilities/send/poll.go`,
`internal/headless/pollreal_test.go`.
...
**Onde**: `internal/headless/capabilities/contacts/about.go`.
...
**Data**: 2026-08-21
**Contexto**: Fase 1, cobertura. Capacidade de **leitura**.
**Onde**: `internal/headless/capabilities/ack/`.
...
**Data**: 2026-08-21
**Contexto**: Fase 1, cobertura. Capacidade de **leitura**.
**Onde**: `internal/headless/capabilities/contacts/labels.go`.
...
**Onde**: `internal/headless/spa/argprobe.go`,
`internal/headless/probe_argshape_test.go`.
...
**Onde**: `internal/headless/capabilities/send/media.go`,
`internal/headless/testdata/lab-sticker.webp`.
...
**Onde**: `internal/headless/LEDGER-WWEBJS.md`,
`internal/headless/gate_ledger_test.go`,
`internal/headless/testdata/wwebjs-v1.34.7-surface.txt`.
...
**Onde**: `internal/headless/capabilities/chats/markunread.go`.
...
**Onde**: `internal/headless/capabilities/group/policy.go`.
...
**Onde**: `internal/headless/capabilities/pin/`.
...
**Onde**: `internal/headless/spa/confirmation.go`,
`internal/headless/probe_writeclass_test.go`.
...
**Onde**: `internal/headless/capabilities/react/react.go`.
...
**Onde**: `internal/headless/events/`, `internal/headless/eventtypesreal_test.go`.
...
na outra, com os nomes `headless-lab A` e `headless-lab B`. É configuração
...
**Onde**: `internal/headless/capabilities/message/` (novo: `message.go`,
`script.go`, `message_test.go`), `internal/headless/probe_msgorigin_test.go`.
...
**Onde**: `internal/headless/capabilities/message/script.go` (`shapeScript`),
...
**Onde**: `internal/headless/capabilities/message/` (`currentScript`,
`Reader.CurrentOf`), `internal/headless/probe_msgmutable_test.go` (novo).
...
**Onde**: `internal/headless/LEDGER-WWEBJS.md`, seção `## Chat`.
...
**Onde**: `internal/headless/capabilities/settings/` (novo),
`internal/headless/probe_settings_test.go` (novo).
...
**Onde**: `internal/headless/capabilities/phone/` (novo),
`internal/headless/spa/webversion.go` (novo),
`internal/headless/probe_settings_test.go`.
...
**Onde**: `internal/headless/capabilities/channel/` (`searchScript`,
`DirectoryEntry`, `Reader.Search`), `internal/headless/probe_chansearch_test.go`.
...
**Onde**: `internal/headless/capabilities/channel/owner.go` e `script.go`
(novos), `internal/headless/probe_chanowner_test.go` (novo).
...
identifiquei pelo prefixo do nome (`headless probe`) e apaguei. Confirmado
...
**Onde**: `internal/headless/capabilities/search/` (novo),
`internal/headless/probe_search_test.go` (novo).
...
**Onde**: `internal/headless/capabilities/liveness/reset.go` e
`resetscript.go` (novos), `internal/headless/probe_reset_test.go` (novo).
...
**Onde**: `internal/headless/AUDITORIA-FACHADA.md` (novo, produzido por
...
**Onde**: `internal/headless/events/group.go` e `group_test.go` (novos),
...
**Onde**: `internal/headless/probe_mediaevent_test.go` (novo),
`LEDGER-WWEBJS.md`.
...
**Onde**: `internal/headless/events/group.go` (tipo `VoteUpdated`),
...
**Onde**: `internal/headless/probe_gp2_test.go`
...
**Onde**: `internal/headless/capabilities/channel/` (`Follow`, `Unfollow`,
`Followed`, `followScript`, `followedScript`),
`internal/headless/probe_follow_test.go` (novo).
...
**Onde**: `internal/headless/probe_reactsource_test.go` (novo),
`probe_gp2_test.go`.
...
**Onde**: `internal/headless/capabilities/group/description.go` e
...
**Onde**: `internal/headless/capabilities/lookup/` (novo),
...
**Onde**: `internal/headless/dualsession_test.go` (novo),
...
**Onde**: `internal/headless/probe_chanadmin_test.go` (novo).
...
**Onde**: `internal/headless/probe_chanadmin_test.go`.
...
**Onde**: `internal/headless/probe_chantransfer_test.go` (novo).
...
**Onde**: `internal/headless/HOUSEKEEP.md` H75 (2026-08-21), e a linha 70 do
...
## H145 — `/session/qr` para headless: falta um boot primitivo inteiro, não só um adapter

**Data**: 2026-08-29. **Contexto**: pedido do usuário para aplicar a mesma
lógica de contrato QR do `noise` (leitura de `users.qrcode`, string crua,
sem renderização — ver `pkg/infra/noise/adapters/pairing/qr.go`) ao
`headless`, depois de consultar como `wwebjs` e `open-wa` resolvem e
renderizam o QR.

**Onde**: `internal/headless/probe_qr_test.go` (novo, `TestProbeQRConstructionSurface`),
`internal/headless/core/session.go:8-10,321-323`,
`internal/headless/runtime/holder.go:121`,
...
`internal/headless/capabilities/`). Os cinco módulos que faltavam
...
`internal/headless/runtime/holder.go:121` chama `core.StartSession`
...
interaja com ela. Construir o `QRReader` de `headless` para
...
- **`core.StartPairingSession`** (`internal/headless/core/session.go`) —
...
- **`Holder.PairingSession`** (`internal/headless/runtime/holder.go`) —
...
- **`Sessions.EvaluatorForPairing`** (`pkg/infra/headless/sessions.go`,
...
`noiseInfo_get_ok: true`, `staticKeyPair_pubKey_present: true`,
...
`internal/headless/capabilities/qr` (o algoritmo, com o comentário do
pacote citando exatamente de onde cada passo vem), `pkg/infra/headless/
...
`headless` quando `s.Headless.ChromePath` está configurado).
...
**Testes**: `internal/headless/core/pairing_session_test.go` (novo —
...
TerminatesFastWithSpecificCause`), revertido. `pkg/infra/headless/
...
`headless`) foram atualizados para o novo — não é enfraquecimento de
...
`go test -race ./pkg/...` e `go test ./internal/headless/...` verdes;
...
faz UMA leitura extra (`headless.RefreshOwnIdentity` — a mesma que
...
**Testes**: `internal/headless/capabilities/qr/qr_test.go` (novo) — 4
...
`go test ./internal/headless/...` verdes; `.log-coverage-baseline`/
