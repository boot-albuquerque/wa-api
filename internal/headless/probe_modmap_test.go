...
func TestProbeModuleRegistry(t *testing.T) {
	...
	const script = `(() => {
		window.__headlessModProbe = { stage: 'pending' };
		...
		(async () => {
			try {
				...
				window.__headlessModProbe = { stage: 'error', why: String((e && e.message) || e) };
			}
		})();
		return 'kicked';
	})()`

	const pollScript = `JSON.stringify(window.__headlessModProbe || {stage:'missing'})`
