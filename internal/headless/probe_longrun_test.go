...
func TestProbeLongRunning(t *testing.T) {
	...
	globals := func() int {
		var raw string
		const s = `(() => { let n = 0; for (const k of Object.keys(window)) { if (k.indexOf("__headless") === 0) { n++; } } return String(n); })()`
	...
	if last := series[len(series)-1]; last.globals > 0 {
		t.Errorf("a pagina terminou com %d global(is) __headless*", last.globals)
	}
}
