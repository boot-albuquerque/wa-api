...
func TestProbeAvatarShape(t *testing.T) {
	...
	const askScript = `(() => {
		window.__headlessAvatarProbe = { stage: 'pending' };
		...
		(async () => {
			...
			window.__headlessAvatarProbe = out;
		})();
		return 'kicked';
	})()`
	...
	for i := 0; ; i++ {
		if err := runner.Do(ctx, engine.OpStateProbe, "probe/avatar/poll", func(c context.Context) error {
			return sess.Tab().Evaluate(c, `JSON.stringify(window.__headlessAvatarProbe || {stage:"missing"})`, &raw)
		}); err != nil {
