...
func TestProbeContactEventSurface(t *testing.T) {
	...
	if err := runner.Do(ctx, engine.OpStateProbe, "probe/contactev/selftest", func(c context.Context) error {
		return sess.Tab().Evaluate(c, `JSON.stringify({selfTestSeen: !!(window.__headlessContactEv||{}).selfTestSeen, allSupported: !!(window.__headlessContactEv||{}).allSupported, err: (window.__headlessContactEv||{}).selfTestErr || ''})`, &raw)
	}); err != nil {
	...
	const script = `JSON.stringify(window.__headlessContactEv || {stage:'missing'})`
