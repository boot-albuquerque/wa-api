...
| `attachEventListeners` | capabilities/messagemeta + contacts.onContact | `PARTIAL` | sim | sim | sim | dois fluxos de 31 eventos. **H171: esta linha é um AGREGADO e não fecha sozinha.** O `attachEventListeners` da referência é uma função que instala 25 ouvintes (`exposeFunctionIfAbsent`), um por evento; o nosso equivalente é o ingresso mais os ouvintes de capacidade. Não há aqui capacidade própria a provar — o estado desta linha é a conjunção das linhas de evento, e ela vira `PROVEN` quando elas virarem. Registrado para que a próxima varredura não gaste uma sonda procurando o que medir |
...
| `sendSeen` | chats.MarkRead | `PARTIAL` | sim | duvidosa | sim | **H160: a pós-condição da H82 agora VALE, porque a primitiva estava errada.** Chamávamos `sendConversationSeen`, e o contador não se mexia — `1 antes, 1 depois`. Medido lado a lado no mesmo chat, o módulo que a referência usa (`WAWebUpdateUnreadChatAction.sendSeen`, com o `Stream.markAvailable` em volta) levou `unreadCount` de 1 a 0. Trocado: `before=1 after=0 changed=true` **nas duas contas**, sem erro. **Continua `PARTIAL` por outro motivo, e este é medido**: o recibo NÃO chega ao remetente. Com a sessão dupla, marcar lida em qualquer direção deixa o ack do outro lado em 2 por 60 s — e a hipótese do `markAvailable` foi testada e REFUTADA. Falhar simetricamente com duas contas torna "privacidade desligada" improvável, mas não a exclui |
...
| `sendReaction` | capabilities/react | `PROVEN` | sim | sim | sim | H155: **as duas metades verificam.** A H53 media certo — `hasReaction` é pegajoso e não volta na sessão que removeu — e a assimetria era falta de FONTE, não de rigor. A H154 achou a fonte; `react.Remove` agora espera pelo registro de reações (`hasReactionByMe`), que DE FATO vira falso. Provado ao vivo: `had=true has=false verified=true`, com o leitor confirmando `groups=0`. *(era: H53: adicionar provado; remover devolve Verified:false — H154: a causa disso caiu — a fonte para "quais reações" existe (`Reactions.find` pelo objeto id) e `message.ReactionsOf` a lê. Verificar o `Remove` passou a ser trabalho ACIONÁVEL, não impedimento; fica registrado como tal em vez de a linha ser fechada por associação |
...
| `markChatUnread` | — | `BLOCKED` | sim | falha (H78) | sim | H140: reclassificado de `MISSING` para `BLOCKED` (decisão 60). A H78 mediu DUAS primitivas e nenhuma marca; o ouvinte do verbo vive num pedaço de UI que sessão headless não carrega. Está fora do alcance deste módulo, não por fazer. **H161: a pista mais específica que esta linha já teve foi gasta e o `BLOCKED` sai REFORÇADO.** A H160 consertou o `MarkRead` trocando de módulo, e o mesmo módulo exporta `markUnread` — candidato óbvio. Testadas TRÊS primitivas no mesmo chat, cada uma medida em separado: `Cmd.markChatUnread(chat,true)` (a da referência), `UpdateUnreadChatAction.markUnread(chat,true)` e a mesma com presença anunciada. **As três retornam sem erro e nenhuma vira `markedUnread`.** A forma da terceira não foi chutada: veio da assinatura lida da própria função — `markUnread(chat, unread, allowAction=true)` |
...
| `getBroadcasts` | status.List | `PARTIAL` | sim | sim | sim | **NÃO é lista de transmissão** — o upstream chama de Broadcast o STATUS (stories): `getBroadcasts` é `Status.getModelsArray`. Nossa nota descrevia a coisa errada, e três linhas iam ser feitas contra a ideia errada (H100). **H158: o zero foi INTERROGADO e é o mundo, não o instrumento.** Três linhas caíram hoje porque um zero significava "ninguém pediu" (H142, H153, H154), então este foi testado do mesmo jeito: `Status.hasSynced()` já era **true ANTES** de qualquer sync, o `sync()` rodou sem erro, o tamanho ficou em 0, e o caminho independente `WAWebApiStatus.getAllStatuses()` devolveu `array[0]` — com 947 contatos na conta. A coleção está hidratada e vazia. Caminho provado ao vivo, com **zero** feeds; provar um não-vazio exige POSTAR status, visível aos 944 contatos da conta |
| `getBroadcastById` | status.ByContact | `PARTIAL` | sim | sim | sim | tenta as duas formas de identidade; feed ausente é erro próprio e não um feed de zeros, que um chamador leria como "essa pessoa não postou nada" (H100). **H158: bloqueado pela mesma medição do `getBroadcasts`** — nenhum dos 947 contatos tem status ativo, e a coleção está comprovadamente sincronizada, então não há feed de ninguém para ler. Não é trabalho parado |
...
| `setDescription` | `BLOCKED` | H140: reclassificado (decisão 60). A página ACEITA e o servidor nunca armazena — medido em canal (H113) e, independentemente, em grupo (H126). Duas superfícies, mesma falha: é comportamento da página, não trabalho pendente. **H163: confirmado do lado que nunca tinha sido perguntado.** A H162 mostrou que "o ator não enxerga" pode ser ponto cego e não bloqueio, e as três medições anteriores desta linha foram TODAS da sessão que agiu. Refeito com sessão dupla: conta-A escreve, conta-B lê o grupo por 60 s — `descLen=0 source="none"`. O `BLOCKED` sai reforçado |
...
| `sendStateRecording` | presence.StateRecording | `PARTIAL` | sim | bloqueada (H50) | sim | **linha corrigida**: eu a marquei MISSING de memória e ela JÁ EXISTIA, mapeada para `markRecording`. A prova ao vivo esbarra no mesmo bloqueio da observação de presença. **H170: o bloqueio foi CONFIRMADO por um caminho independente da assinatura.** A hipótese era que o estado de conversa pudesse chegar ao modelo do par sem passar pela assinatura que a H144 mediu como inalcançável — conta-A anuncia gravação, conta-B lê o modelo de presença cru. Nada se moveu em 45 s (`hasChatstate:true type:""`, coerente com a H94: `chatstate.type` é indefinido neste build). É a mesma dependência de AGENDA da H144: ação do telefone, não trabalho pendente |
...
| `setDescription` | group.SetDescription | `BLOCKED` | sim | medido | sim | **H163: reconferido com sessão dupla e o `BLOCKED` sai REFORÇADO** — conta-A escreve, conta-B não vê em 60 s (`descLen=0 source="none"`). É o contrapeso da H162: nem todo "o ator não enxerga" é ponto cego. H126: implementado e medido. A chamada é ACEITA e o servidor nunca armazena — 0 bytes depois de 20s de espera. É o MESMO comportamento que a descrição de CANAL mostrou na H113: duas superfícies independentes, escrita de descrição que não persiste neste build. O código fica no lugar porque a diferença é da PÁGINA |
...
| `react` | capabilities/react | `PROVEN` | sim | sim | sim | H155: as duas metades AGORA são iguais, e não por afrouxar o critério: a causa da assimetria caiu (H154). `Add` espera pela flag pegajosa, `Remove` espera pelo registro — sinais diferentes para perguntas diferentes, e o `Result` continua dizendo qual foi qual. *(era: H53: `Add` verifica a pós-condição e `Remove` não. *(era só a referência `H53`, expandido na H134)* |
| `acceptGroupV4Invite` | — | `BLOCKED` | — | medido | — | H125: bloqueio DUPLO, medido. `WAWebGroupInviteV4Job` existe mas **nenhuma** das duas funções que a referência chama existe nele — décimo desencontro com a lista do wwebjs. E a conta tem zero convites v4 |
...
| `unpin` | pin.Unpin | `PARTIAL` | sim | entre sessões | sim | **H172: NÃO está bloqueado, e a justificativa antiga era herdada de um veredito refutado.** Dizia "idem `pin`", e a H162 mostrou que a H81 media o pin do lado cego. Remedido: conta-A fixa e conta-B (aberta) VÊ; conta-A desfixa e conta-B aberta continua vendo por **5 minutos** — mas uma sessão de conta-B RECÉM-ABERTA lê **0** logo depois. O unpin chega ao SERVIDOR e não ao modelo de sessão já aberta. `PARTIAL` pela convenção do `addParticipants`/`pin`: confirmação só entre sessões |
...
| `getReactions` | message.ReactionsOf | `PROVEN` | sim | sim | sim | H154: **duas medições certas, duas conclusões erradas, e a causa era a CHAVE.** A H83 disse que não há fonte; a H124 remediu com instrumento melhor, achou `Reactions` com `getModelsArray` em 0 depois de uma reação verificada, e concluiu que a coleção não enche. Ela não enche mesmo — mediu-se 0 de novo hoje. Mas `Reactions.find` não é busca nos modelos carregados: é **fetch assíncrono**, e a referência a aguarda. E a chave dela não existe aqui: medidos lado a lado numa mensagem com reação, `find(_serialized)` LANÇA "called find without an id" (é nulo neste build), `find(id.id)` devolve `null`, e `find(id)` — o OBJETO — devolve o registro. Provado por TRANSIÇÃO: `groups=1 senders=1` após reagir, `groups=0 senders=0` após retirar |
...
| `CHAT_REMOVED` | events.ChatRemoved | `PROVEN` | **H173: a justificativa caiu com a H166 e a linha fechou.** Dizia que provar exigiria apagar uma conversa e destruir o fixture — e a H166 apagou uma, num grupo descartável. Medido primeiro: o apagamento produzia **14 `chat.changed`** e nada dizendo que a conversa SUMIU, porque o ingresso abria só a porta `change` da `ChatCollection`. Aberta a porta `remove` — o mesmo que a H143 fez para mensagens —, o apagamento passa a emitir `chat.removed` **1 vez**, provado ao vivo |
...
| `MESSAGE_REACTION` | events.MessageReaction | `PARTIAL` | disparado ao vivo (H87); diz que as reações se moveram e NÃO quais são. **H154: "o agregado não tem fonte neste build" está REFUTADO** — a fonte é `Reactions.find` pelo objeto id, e `message.ReactionsOf` a lê. Um assinante já pode perguntar quais são ao receber o evento; enriquecer o próprio evento é trabalho acionável e não impedimento |
...
| `DISCONNECTED` | events.SessionStateChanged | `INTENTIONAL_DIFFERENCE` | **decisão 64 (b)**: reclassificada de `PARTIAL`. A capacidade É entregue — emitimos desde a H88, antes só detectávamos —, por um desenho DIFERENTE e deliberado: uma TRANSIÇÃO de liveness com a classe da página anexada, em vez do motivo de desligamento do upstream. A H168 estreitou a distância ao fazer a classe VIAJAR no evento. O que resta é escolha nossa e registrada, que é a definição desta coluna |
| `STATE_CHANGED` | events.SessionStateChanged | `INTENTIONAL_DIFFERENCE` | **decisão 64 (b)**: reclassificada de `PARTIAL`. Emitimos só na TRANSIÇÃO, e o motivo está escrito: repetir "ainda vivo" a cada tique é heartbeat vestido de evento. O vocabulário é o nosso (`ALIVE`, `PROCESS_GONE`, `APP_ABSENT`, …) e não o estado de socket do upstream — decisão de desenho, tomada e registrada, não medição pendente |
