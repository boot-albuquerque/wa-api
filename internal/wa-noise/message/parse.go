...
//
// `requireParticipant` faz o atributo `participant` de grupo/broadcast virar
// obrigatorio (erro se ausente) em vez de opcional.
func ParseSource(t Transport, node *waBinary.Node, requireParticipant bool) (source types.MessageSource, err error) {
	clientID := t.OwnID()
	clientLID := t.OwnLID()
	if clientID.IsEmpty() {
...
// ParseInfo monta o types.MessageInfo completo de um stanza <message>.
//
// `user.ParseVerifiedNameContent` e' chamada por IMPORT DIRETO, e nao pela
// fachada da raiz: e' funcao pura (Node -> *types.VerifiedName), sem
// transporte nem estado. Isso fecha a divida que o lote 7 deixou anotada em
// user.go ("usado por message_parse.go, que continua na raiz ... lote 9,
// pendente").
func ParseInfo(t Transport, node *waBinary.Node) (*types.MessageInfo, error) {
	var info types.MessageInfo
	var err error
	info.MessageSource, err = ParseSource(t, node, true)
	if err != nil {
		return nil, err
	}
	ag := node.AttrGetter()
	...
	for _, child := range node.GetChildren() {
		...
		case "multicast":
			info.Multicast = true
		case "verified_name":
			info.VerifiedName, err = user.ParseVerifiedNameContent(child)
			if err != nil {
				t.Log().Warnf("Failed to parse verified_name node in %s: %v", info.ID, err)
			}
		case "bot":
			info.MsgBotInfo, err = ParseBotInfo(child)
			if err != nil {
				t.Log().Warnf("Failed to parse <bot> node in %s: %v", info.ID, err)
			}
		case "meta":
			info.MsgMetaInfo, err = ParseMetaInfo(child)
			if err != nil {
				t.Log().Warnf("Failed to parse <meta> node in %s: %v", info.ID, err)
			}
		case "franking":
			// TODO
		case "trace":
