# Patches locais em internal/noise/

Registro do que divergimos do upstream `wa-api/internal/noise` e por quê.
...
uma falha. Só `internal/noise/proto/` (código gerado a partir de `.proto`)
...
### Nota de época — caminhos de `pkg/infra/noise/` (F61)

As entradas anteriores a 2026-08-07 citam subdiretórios de
`pkg/infra/noise/` na estrutura plana que existia até o commit `ca34600`
...
|---|---|
| `pkg/infra/noise/walog/` | `pkg/infra/noise/observability/walog/` |
| `pkg/infra/noise/group/` | `pkg/infra/noise/adapters/group/` |
| `pkg/infra/noise/user/` | `pkg/infra/noise/adapters/user/` |
| `pkg/infra/noise/safego/` | `pkg/infra/noise/runtime/safego/` |

---

## Fase A — raiz do pacote (`internal/noise/*.go`), 2026-08-06
...
`pkg/infra/noise/` (commits `e2c6206`, `84e10d3`, `4ebef07`, `59b243e`,
...
(`package noise`) e do **mesmo diretório** — nenhuma assinatura pública,
...
- `internal/noise/proto/` — código gerado, ADR-0004 decisão 1.
  `git diff --stat internal/noise/proto/` está vazio e continua travado por
  `make waclient-drift`.
- `internal/noise/internals.go` (756 linhas) — o arquivo declara
...
- **Por quê**: `user.go` era o maior catch-all do pacote — links/QR, bots,
  ...
  adotado em `pkg/infra/noise/user_adapters.go` (commit `59b243e`), que
...
- **Por quê**: mesmo eixo do split de `pkg/infra/noise/group_adapters.go`
...
- **Auditoria de logging (raiz do pacote inteira, não só `client.go`)**:
  varrido `internal/noise/*.go` atrás de qualquer escrita de log que não
  ...
  zerolog (`pkg/infra/noise/walog/`, commits `2848388` e `279a68a`).
  Resultado: o noise usa `cli.Log` corretamente em toda a raiz, com **uma
...
- **O que faz**: falha se qualquer `.go` de produção da **raiz** de
  `internal/noise/` passar de 300 linhas. Isenta `*_test.go` e qualquer
  arquivo cuja primeira linha contenha `GENERATED BY` (hoje só
  `internals.go`).
- **Por quê um script e não um `_test.go`**: `internal/noise/` está fora de
...
## Fase A — `internal/noise/socket/`, 2026-08-06
...
Varrido `internal/noise/socket/*.go` atrás de escrita de log fora do
...
(`pkg/infra/noise/walog/`, commits `2848388`/`279a68a`). `NoiseSocket` loga
...
`internal/noise/`; o único `_test.go` do fork inteiro é
...
- `scripts/waclient-filesize-check.sh` passou de um diretório (`DIR`) para uma
  lista (`DIRS`), agora cobrindo `internal/noise/` **e**
  `internal/noise/socket/` — as duas metades da Fase A. A checagem de "achei
...
- **Criado** o alvo `make waclient-test`, ligado a `check`. Pelo mesmo racional
  do gate de tamanho: `internal/noise/` está fora de `TEST_PKGS`
...
- `cmd/logcov/testdata/eligible.golden` ganhou duas linhas
  ...
  `internal/noise/` está em `.logcov-exclude`) — mesma regeneração mecânica
  do commit `5d376a6`.

### Fora do escopo

- `git diff --stat internal/noise/proto/` continua vazio.
...
## Fase B — `internal/noise/appstate/`, 2026-08-06
...
Varrido `internal/noise/appstate/*.go` atras de escrita de log fora do
...
sobre zerolog (`pkg/infra/noise/walog/`). Nada a corrigir, e nenhuma
infraestrutura de log nova foi adicionada.

### Cobertura de teste

O diretorio nao tinha **nenhum** `_test.go`. Cobertura final: **91.0% de
statements** (`go test -race -cover ./internal/noise/appstate/`).
...
`internal/noise/store/` foi tocado.**
...
- `scripts/waclient-filesize-check.sh`: `DIRS` ganhou
  `internal/noise/appstate/`. Nenhum arquivo do diretorio tem cabecalho de
  codigo gerado, entao a isencao continua valendo so' para `internals.go`.
- `Makefile`: `WACLIENT_TEST_PKGS` ganhou `./internal/noise/appstate/`, para
  que os `_test.go` acima rodem de fato em `make check` — `internal/noise/`
  segue fora de `TEST_PKGS` (achado F17).

### Fora do escopo

- `git diff --stat internal/noise/proto/` continua vazio.
- `internal/noise/appstate/lthash/` (58 linhas, subpacote proprio) nao foi
...
- `internal/noise/store/` — segunda metade da Fase B, executada em paralelo.

---

## `internal/noise/argo/`, 2026-08-06
...
- Único consumidor no repo: `internal/noise/newsletter_mex.go:18`. Não
...
## Fase B — `internal/noise/store/` e `store/sqlstore/`, 2026-08-06
...
- **Criado** `store_identity.go` (52): as 4 queries de
  `wanoise_identity_keys` e `PutIdentity`, `DeleteAllIdentities`,
  `DeleteIdentity`, `IsTrustedIdentity` (`store.IdentityStore`).
- **Criado** `store_session.go` (120): as queries de `wanoise_sessions`,
...
- **Criado** `store_prekey.go` (128): as 7 queries de `wanoise_pre_keys`,
...
- **Por que**: `store.go` misturava **duas coisas de natureza diferente** — o
  ...
  implementa e que o resto do noise consome) com a unica **implementacao
...
- **As queries SQL nao foram fragmentadas em constantes de nome de tabela e
  ...
  `SELECT session FROM wanoise_sessions WHERE our_jid=$1` por concatenacao de
  ...
  excecao sao os nomes de coluna de `wanoise_chat_settings`, que **ja' eram**
...
Varrido `internal/noise/store/` e `store/sqlstore/` atras de escrita de log
...
- `sqlstore`: `s.log.Infof`/`Debugf` em `MigratePNToLID` e `s.log.Warnf` em
  ...
  ja' ligado ao `walog.Bridge` sobre zerolog (`pkg/infra/noise/walog/`).
...
| Pacote | Cobertura |
|---|---|
| `internal/noise/store` | **98.6%** de statements |
| `internal/noise/store/sqlstore` | **89.7%** de statements |
...
- **Traducao LID <-> PN** nos `CASE` de `getMsgSecret` e `getPrivacyToken`, nas
  duas direcoes, com o `wanoise_lid_map` populado.
...
- `scripts/waclient-filesize-check.sh`: `DIRS` ganhou `internal/noise/store/`
  e `internal/noise/store/sqlstore/`. Nenhum arquivo dos dois diretorios tem
  cabecalho de codigo gerado, entao a isencao continua valendo so' para
  `internals.go`. O gate passa a cobrir 138 arquivos.
- `Makefile`: `WACLIENT_TEST_PKGS` ganhou `./internal/noise/store/` e
  `./internal/noise/store/sqlstore/` — `internal/noise/` segue fora de
...
- `git diff --stat internal/noise/proto/` continua vazio.
- **`internal/noise/store/sqlstore/upgrades/*.sql` nao foi tocado.** Migracao
  de schema e' historico append-only: cada arquivo ja' foi aplicado em bancos de
  producao e esta' registrado em `wanoise_version`. Editar, dividir ou
...
- `PostgresArrayWrapper` como variavel global de pacote mutavel e' o mesmo cheiro
  ...
  (`noise.PostgresArrayWrapper = pq.Array`), mudar quebraria consumidores.
...
## Fase C — `internal/noise/binary/`, `types/`, `util/`, 2026-08-06
...
**dentro do commit `c196a68`** ("refactor(infra/noise): extrai
...
```
internal/noise/binary/{constants,decoder,decoder_node,encoder,encoder_node,jid,packing,unpack}.go
internal/noise/types/events/{events,failure_reasons,group,message,newsletter,user}.go
```
...
- `util/log/` **é** a infraestrutura de log. `stdoutLogger` usa `fmt.Printf`
  ...
  que o `pkg/infra/noise/walog/` do wa-api usa para plugar o zerolog do
  projeto.
...
| Pacote | Cobertura |
|---|---|
| `internal/noise/binary` | **97.1%** |
| `internal/noise/types` | **99.2%** |
| `internal/noise/types/events` | **100%** |
| `internal/noise/util/cbcutil` | **91.7%** |
| `internal/noise/util/gcmutil` | **92.9%** |
| `internal/noise/util/hkdfutil` | **75.0%** |
| `internal/noise/util/keys` | **100%** |
| `internal/noise/util/log` | **100%** |
...
- `Makefile`: `WACLIENT_TEST_PKGS` ganhou os oito pacotes novos, chegando a
  12. `internal/noise/` segue fora de `TEST_PKGS` (achado F17), então sem
  isso os `_test.go` desta fase nunca rodariam em `make check` — seriam
  travas que não travam.

### Fora do escopo

- `git diff --stat internal/noise/proto/` continua vazio. **E
  `internal/noise/binary/proto/` também não foi tocado**: `legacy.go`
...
## Decisão — a raiz de `internal/noise/` **não** vira subpacotes, 2026-08-06
...
Estender à raiz de `internal/noise/` a mesma reorganização por domínio que
`pkg/infra/noise/` acabou de receber (73 arquivos planos → subpacotes
`session/`, `group/`, `user/`, `chat/`, ...): "organização extrema, sem
arquivos `.go` jogados na raiz".

### Por que a mesma operação não se aplica aqui

`pkg/infra/noise/` é código **nosso**, e seus 73 arquivos eram adaptadores
...
A raiz de `internal/noise/` é o oposto: é **um único pacote coeso
...
- `download_types.go`, `errors.go`, `msgsecret_keys.go`, `send_types.go`,
  `send_debug_timings.go` declaram **API exportada** (`noise.MediaType`,
  `noise.SendResponse`, `noise.SendRequestExtra`,
  `noise.ElementMissingError`, `noise.MsgSecretType`) que **45
...
("a raiz de `internal/noise/` **não** vira subpacotes"). O usuário foi
...
subpacote — `noise.RemoveReactionText` → `msgattrs.RemoveReactionText`
(`msgattrs/message.go:142`) e os quatro `noise.Adv*SignaturePrefix` →
...
`internal/noise/`: `RemoveReactionText` só era usado dentro do próprio
...
Medido, são **249 chamadas** fora de `internal/noise/` para **29 métodos
...
A Fase A quebrou a raiz de `internal/noise/` em arquivos por domínio, mas
...
`pkg/infra/noise/walog/`): `download.go:163`, `download-to-file.go:108`
...
Sete arquivos novos, todos em `package noise` (interno, para alcançar
`getMediaKeys`, `validateMedia`, `shouldRetryMediaDownload` e afins);
`client_test.go`, que é `package core_test` e só tem um `Example`,
...
`WACLIENT_TEST_PKGS` no `Makefile` ganhou `./internal/noise/` (a raiz).
...
`scripts/waclient-filesize-check.sh` já listava `internal/noise` em `DIRS`;
`media_constants.go` (o único arquivo de produção novo) tem 76 linhas e carrega
o header MPL-2.0. `git diff --stat internal/noise/proto/` continua vazio.
...
Quatro arquivos novos, `package noise`:
...
`WACLIENT_TEST_PKGS` já inclui `./internal/noise/` desde o lote 1 — nada a
...
da cola de app state que vive na **raiz** do pacote `noise`:

`appstate.go`, `appstate_dispatch.go`, `appstate_keys.go`, `appstate_send.go`.

Não confundir com o subpacote `internal/noise/appstate/`, coberto na Fase B
e **não tocado aqui** (`git diff --stat internal/noise/appstate/` vazio).
...
Três arquivos novos, `package noise`:
...
`WACLIENT_TEST_PKGS` já inclui `./internal/noise/` desde o lote 1 — nada a
mexer no `Makefile`. `git diff --stat internal/noise/proto/` e
`git diff --stat internal/noise/appstate/` continuam vazios.
...
a mesma ponte para zerolog montada fora do fork em `pkg/infra/noise/walog/`.
...
Cinco arquivos novos, todos `package noise`.
...
raiz, todos `package noise`, agrupados em três concerns vizinhos mas
distintos:
...
(`pkg/infra/noise/walog/`). Não há `fmt.Print*`, `log.*` nem `println`.
...
Três arquivos novos, todos `package noise`. A infraestrutura compartilhada
...
grupo da **raiz** do pacote, todos `package noise`:
...
Nada a ver com `pkg/infra/noise/group/`, que é a camada de adaptação do
...
(`pkg/infra/noise/walog/`). Não há `fmt.Print*`, `log.*`, `println` nem
...
(`collectParticipantList`, `EXCLUDED X5`, já que `internal/noise/` está em
...
usuário da **raiz** do pacote, todos `package noise`:
...
Nada a ver com `pkg/infra/noise/user/`, que é a camada de adaptação do
projeto e já foi feita em outra frente.
...
`walog.Bridge` sobre zerolog (`pkg/infra/noise/walog/`). Não há `fmt.Print*`,
...
Cinco arquivos novos, todos `package noise`. A infraestrutura compartilhada
...
que `internal/noise/` está em `.logcov-exclude`) — mesma regeneração mecânica
dos lotes anteriores. `git diff --stat internal/noise/proto/` continua vazio,
...
do caminho de **saída** da **raiz** do pacote, todos `package noise`:
...
Onze arquivos da raiz, todos `package noise`: `message.go`,
...
Nove arquivos da raiz, todos `package noise`: `client.go`,
...
Com o lote 10 a Fase E cobre os 90 arquivos da raiz de `internal/noise/`.
...
de `internal/noise/` **não** vira subpacotes" e no que a Fase D conseguiu e
...
adicionar log e teste, corrigir bugs — sempre dentro de `package noise`, sem
...
`internal/noise/media/`, e a raiz virou fachada fina.
...
Em `internal/noise/media/transport.go`, duas interfaces, deliberadamente
pequenas:
...
**Criados** (`internal/noise/media/`, `package media`): `constants.go`,
...
site fora de `internal/noise/`** — os 249 pontos de chamada em
`pkg/infra/noise/` e afins não foram tocados. `internals.go` (gerado) também
...
`internal/noise/media/` fecha em **98,8% de statements e 100% de funções**
...
`scripts/waclient-filesize-check.sh` (`DIRS`) e `WACLIENT_TEST_PKGS` no
`Makefile` passaram a incluir `internal/noise/media`. Todos os 11 arquivos de
...
`internal/noise` para `internal/noise/media`.
...
- `go build ./...`, `go vet ./internal/noise/...` e
  `go test -race -count=1 ./internal/noise/media/... ./internal/noise/...`
...
Em `internal/noise/newsletter/transport.go`, uma interface só, com dez
...
2. **`ElementMissing(tag, in string) error` em vez de mover
   ...
   `*noise.ElementMissingError`, bit-a-bit o de antes. Mesma classe de
...
**Criados** (`internal/noise/newsletter/`, `package newsletter`):
...
`go build ./...` do repositório inteiro passa **sem uma única alteração de call
site fora de `internal/noise/`** — `pkg/infra/noise/` e afins não foram
tocados.
...
`internal/noise/newsletter/` fecha em **88,2% de statements e 100% de
...
`scripts/waclient-filesize-check.sh` (`DIRS`) e `WACLIENT_TEST_PKGS` no
`Makefile` passaram a incluir `internal/noise/newsletter`. Todos os 8
...
`internal/noise/appstate/` **já existe** desde a Fase B e é outra coisa: é a
...
`appstatesync/` importa `appstate/`, nunca o contrário. `internal/noise/appstate/`
**não foi tocado neste lote** — `git diff --stat internal/noise/appstate/`
continua vazio, como `internal/noise/proto/`.

### A interface

Em `internal/noise/appstatesync/transport.go`, uma interface com treze
métodos:
...
3. **`ElementMissing(tag, in string) error` em vez de mover
   ...
   `*noise.ElementMissingError`, bit-a-bit o de antes; só a construção
...
**Criados** (`internal/noise/appstatesync/`, `package appstatesync`):
...
`internal/noise/appstatesync/` fecha em **100,0% de statements e 100% de
...
`scripts/waclient-filesize-check.sh` (`DIRS`) e `WACLIENT_TEST_PKGS` no
`Makefile` passaram a incluir `internal/noise/appstatesync`. Todos os 9
arquivos de produção do novo pacote ficam abaixo do teto de 300 linhas (maior:
...
`git diff --stat internal/noise/proto/` e
`git diff --stat internal/noise/appstate/` continuam **ambos vazios**.
...
1. **A falta de `-race`.** Rodado explicitamente:
   `go test -race -count=1 ./internal/noise/appstatesync/... ./internal/noise/`
...
2. **A falta de diff linha-a-linha.** Feito um diff normalizado — whitespace e
   ...
   `git show <commit>^:internal/noise/appstate_dispatch.go` e
...
- `internal/noise/appstate/` (o subpacote da Fase B) não foi tocado.
...
|---|---|---|
| `internal/noise/prekeys/` | `prekeys.go`, `prekeys_constants.go` | **100,0%** |
| `internal/noise/pairing/`  | `pair.go`, `pair-code.go`, parte de `pair_constants.go` | **97,6%** |
| `internal/noise/tctoken/`  | `tctoken.go`, parte de `token_constants.go` | **100,0%** |
...
### Subpacote 1 — `internal/noise/prekeys/`
...
- **Item 6 — `go vet ./internal/noise/...`**: rodado pelo revisor, saída
  vazia, exit 0.
...
- Não checou chamadores da API de pareamento fora de `internal/noise/`; a
...
### Subpacote 3 — `internal/noise/tctoken/`
...
`git diff --stat internal/noise/proto/` **vazio**.
...
| Subpacote | Arquivos de raiz absorvidos | Cobertura (statements, sob `-race`) |
|---|---|---|
| `internal/noise/notification/` | parte de `notification.go`, `notification_newsletter.go`, parte de `notification_privacy.go`, `notification_constants.go` | **100,0%** |
| `internal/noise/retry/` | `retry.go`, `retry_recent_messages.go`, `retry_request_from_phone.go`, `retry_receipt_send.go`, `retry_constants.go` | **98,1%** |

---

### Subpacote 1 — `internal/noise/notification/`
...
### Subpacote 2 — `internal/noise/retry/`
...
`go vet ./internal/noise/...` limpo, e a comparação manual de cada função
movida contra o original — feita **por quem escreveu o commit**, e portanto
**não** independente.
...
| Subpacote | Arquivos de raiz absorvidos | Cobertura (statements, sob `-race`) |
|---|---|---|
| `internal/noise/group/` | `group.go`, `group_constants.go`, `group_create.go`, `group_invite.go`, `group_notification.go`, `group_parse.go`, `group_participants.go`, `group_settings.go` | **100,0%** |
...
- `TrimMessageIDPrefix(id) string` — `Create` remove `WebMessageIDPrefix` da
  ...
  2-5.
- `ElementMissing(tag, in) error` — preserva o `*noise.ElementMissingError`
...
`internal/noise/`** — verificado. `pkg/infra/noise/group/` (a camada de
projeto, um projeto diferente e já pronto), que usa `noise.ReqCreateGroup`,
...
`internal/noise/group/`: **100,0% dos statements**, sob `-race`, medido com
`go test -race -count=1 -cover ./internal/noise/group/`.
...
**`pkg/infra/noise/group/` — outro projeto, outra camada.** É a camada de
...
| Subpacote | Arquivos de raiz absorvidos | Cobertura (statements, sob `-race`) |
|---|---|---|
| `internal/noise/user/` | `user.go`, `user_avatar.go`, `user_blocklist.go`, `user_bots.go`, `user_business.go`, `user_constants.go`, `user_devices.go`, `user_links.go`, `user_usync.go` | **100,0%** |
...
`go build ./...` no repositório inteiro passa **sem uma única mudança fora de
`internal/noise/`** — verificado. `git diff HEAD~2 --stat -- pkg/` vazio:
`pkg/infra/noise/user/` (a camada de projeto, um projeto diferente e já
...
`internal/noise/user/`: **100,0% dos statements**, sob `-race`, medido com
`go test -race -count=1 -cover ./internal/noise/user/`.
...
**`pkg/infra/noise/user/` — outro projeto, outra camada.** É a camada de
...
São os 11 arquivos de envio da raiz, todos `package noise`:
...
- `group` — pelos tipos `*group.Meta` (retorno de `CachedGroupData`) e
  `group.ErrNotFound` (o valor por trás de `noise.ErrGroupNotFound`).
...
faz dos dois o **mesmo** tipo, então quem escrevia `noise.SendResponse{...}`
...
**77,1%** de statements em `internal/noise/send/` (medido, não estimado).
...
`scripts/waclient-filesize-check.sh` ganhou `internal/noise/send` em `DIRS`;
`WACLIENT_TEST_PKGS` (Makefile) ganhou `./internal/noise/send/`. Todos os 14
...
exato do lote 8. São os 11 arquivos da raiz, todos `package noise`:
...
**83,5%** de statements em `internal/noise/message/` (medido, não estimado) —
...
`scripts/waclient-filesize-check.sh` ganhou `internal/noise/message` em
`DIRS`; `WACLIENT_TEST_PKGS` (Makefile) ganhou `./internal/noise/message/`.
...
Último lote da Fase F/G. Nove arquivos da raiz, todos `package noise`:
...
Medido com `grep -n socketLock internal/noise/*.go`, os arquivos de produção
...
```go
// internal/noise/handshake.go (fachada, 33-46)
...
**nenhum** tem consumidor fora de `internal/noise/handshake.go`:

| Antes | Agora |
|---|---|
| `noise.NoiseHandshakeResponseTimeout` | `handshake.ResponseTimeout` (a raiz mantém uma **const** reexportada, `handshake.go:19`) |
| `noise.WACertIssuerSerial` | `handshake.WACertIssuerSerial` (removida da raiz) |
| `noise.WACertPubKey` | `handshake.WACertPubKey` (removida da raiz) |
...
não tipos novos: `noise.SetProxyOptions` aparece nas assinaturas de
`SetProxy`/`SetSOCKSProxy`/`SetProxyAddress` e é usado fora do fork, em
`pkg/infra/noise/session/provider.go:38-39` e
`pkg/infra/noise/session/session.go:83,86`. Um tipo distinto quebraria esses
quatro pontos.
...
O propósito do arquivo **é ser `noise.ErrX`**. Movê-lo para
`internal/noise/errors/` não reduziria acoplamento nenhum — só trocaria o
nome do pacote na única coisa que ele faz, e quebraria os consumidores que já
usam os nomes históricos (`pkg/infra/noise/session/session.go` usa
`noise.ErrQRStoreContainsID`, `noise.ErrProfilePictureUnauthorized`,
`noise.ErrProfilePictureNotSet`).
...
Comparado `git show HEAD:internal/noise/handshake.go:121-128` com
`internal/noise/handshake.go:33-46`. Entre o retorno de `nh.Finish` e a
...
`grep -rn 'noise"' internal/noise/{handshake,keepalive,proxyconf}/` não
...
ganharam `internal/noise/handshake`, `internal/noise/keepalive` e
`internal/noise/proxyconf`. O gate passa a cobrir **298 arquivos de produção
...
→ `internal/noise/core/`. Um único `git mv`, sem nenhuma edição de conteúdo
dos arquivos movidos.

Não houve como fazer o movimento em grupos lógicos com `go build` entre cada um,
como a etapa previa: os 114 arquivos são **um único pacote Go** (`noise`,
exceto `client_test.go` que é `core_test`). Mover metade deixaria o mesmo
`package noise` declarado em dois diretórios — dois pacotes distintos para o
...
`internal/noise/` deixou de ter qualquer `.go` na raiz. O `main.go`-fachada
...
### Nome do pacote: mantido `noise` (não renomeado para `core`)

`internal/noise/core/` declara `package noise`. Go não exige que o nome do
...
- mantém `noise.Client` como identificador em todos os call sites de
  `pkg/infra/noise/*` e `pkg/infra/history/`;
...
reescrever todas as referências `noise.X` → `core.X` nos 45 consumidores —
blast radius incompatível com "preservar API externa, minimizar risco".

Consequência: os imports em `pkg/` ficam com path `…/noise/core` e
identificador `noise`. Isso é legal em Go e o `vet`/`lint` passam, mas é uma
divergência nome-vs-diretório que a etapa 6 deve resolver de vez (o `main.go`
fachada em `internal/noise/` volta a ser `package noise` e os consumidores
voltam a importar a raiz).

### Invariante de direção de import — verificada

```
$ grep -rn 'noise/core' internal/noise/capabilities/ | wc -l
0
$ grep -rn 'noise/core' internal/noise/ --include='*.go' | grep -v '^internal/noise/core/'
(vazio)
```

Nenhum pacote dentro de `internal/noise/` fora de `core/` importa `core/` —
...
Os 44 importadores de `core/` são todos externos ao fork (`pkg/infra/noise/*`,
...
| Arquivo | Mudança |
|---|---|
| `scripts/waclient-filesize-check.sh:36` | `DIRS="internal/noise \` → `internal/noise/core \` |
| `Makefile:263` | `WACLIENT_TEST_PKGS := ./internal/noise/ \` → `./internal/noise/core/ \` |
| `cmd/logcov/testdata/eligible.golden` | regenerado: 2670 linhas antes e depois, diff **puramente** o rename `internal/noise.X` → `internal/noise/core.X`; todas continuam `EXCLUDED X5` |

`.logcov-exclude` **não** precisou de mudança: a entrada é o prefixo
`internal/noise/`, que já cobre `internal/noise/core/`.
`Makefile:17` (`COVER_PKGS`) e `:30` (`LINT_TARGETS`) também não: filtram por
`^wa-api/internal/noise`, prefixo que segue válido. Os `DEST="internal/noise"`
...
"não dá para mover limpo" apareceu; nenhum ciclo novo; `internal/noise/*.go`
...
A etapa 6 cria `internal/noise/main.go` (`package noise`) como fachada do
...
alias — 40 como `noise`, 4 como `wa` — então um `grep core\.` ingênuo não
...
- "Não faça breaking changes gratuitas" não se aplica aqui, por dois motivos
  ...
  `noise "wa-api/internal/noise/core"` para
  `noise "wa-api/internal/noise"` deixa todos os seletores
  `noise.Client`, `wa.NewClient` etc. **idênticos**. O diff é de 44 linhas de
  import e nada mais.
...
justifica o alias dizendo que "`noise.SetProxyOptions` aparece nas
assinaturas".
...
`internal/noise/` importar `wa-api/internal/noise/core`, e falha se
...
| Arquivo | Mudança |
|---|---|
| `internal/noise/main.go` | **novo** — fachada, `package noise`, 20 símbolos |
| `scripts/waclient-facade-check.sh` | **novo** — trava de import |
| `Makefile` | alvo `waclient-facade` + entrada em `check` |
| `scripts/waclient-filesize-check.sh:36` | `DIRS` ganha `internal/noise` (não-recursivo → pega `main.go`) |
...
- `Makefile:17/30` (`COVER_PKGS`, `LINT_TARGETS`): filtram
  `grep -v '^wa-api/internal/noise'`, prefixo que já casa com o pacote raiz.
- `.logcov-exclude`: a entrada `internal/noise/` cobre o pacote raiz porque
  `ruleX5PackageExcluded` (`cmd/logcov/rules.go:24`) compara também com
  `strings.TrimSuffix(pref, "/")` — `internal/noise` bate exato.
...
- `waclient-{vendor,diff,license-check}.sh`: `DEST="internal/noise"` segue
...
```
internal/noise/keepalive/  ->  internal/noise/runtime/keepalive/
internal/noise/proxyconf/  ->  internal/noise/runtime/proxy/
```
...
runtime/proxy/proxyconf.go          (nenhum import de noise)
```
...
- `scripts/waclient-filesize-check.sh`: `DIRS` repontado para
  `internal/noise/runtime/{keepalive,proxy}`, mais atualização do comentário
...
- `.logcov-exclude`: **não** precisou mudar. A entrada é o prefixo
  `internal/noise/`, que cobre `runtime/` por construção.
- `scripts/waclient-facade-check.sh`: não precisou mudar — a trava é sobre
  `core`, e nada em `runtime/` o importa.

### Os quatro documentos

Criados em `internal/noise/docs/`, ao lado do `FASE_H_INVENTORY.md` da etapa 1:
...
| # | O que fez | Commits |
|---|---|---|
| 1 | **Inventário factual** de `internal/noise/` antes de mover nada — 114 arquivos da raiz, mapa de locks, mapa de call sites cruzados, checagem de ciclos, e 7 decisões pendentes com recomendação. Virou o contrato de execução das etapas 3-7 | `5924304` |
...
| 6 | **Fachada raiz**: `main.go` (`package noise`) sobre `core/`, com `type Client = core.Client` — alias de tipo, que entrega o method set inteiro (inclusive os 178 wrappers de `DangerousInternals`) numa linha. Os 44 consumidores repontados para a fachada, e a regra travada por `scripts/waclient-facade-check.sh` | `741470f`, `0cb4e67`, `bfe5ae2` |
...
```
internal/noise/
├── main.go               fachada, package noise — única porta de entrada
...
CAP-02 (POST /chat/send/image, ramo URL) precisa que `pkg/infra/noise/
client/client.go` (interface estreita, ADR-001) exponha `Client.Upload`.
`*noise.Client` (o alias de tipo completo em `main.go`) já satisfaz esse
método por promoção — mas o CHAMADOR precisa nomear `noise.MediaType` e
`noise.MediaImage` para invocá-lo, e nenhum dos dois estava na fachada
(`internal/noise/main.go`), só em `core` (`internal/noise/core/
...
`internal/noise/main.go`, seção "Perfil e midia": acrescentados os
...
de "alterar o noise" — mas o arquivo tocado (`internal/noise/
...
**`internal/noise/protocol/appstate/decode.go`**: `DecodePatches`,
...
**`internal/noise/protocol/appstate/keys.go`**: `Processor` e
`NewProcessor` não mudam de assinatura. O parâmetro é passado por chamada, não
guardado no processador.

**`internal/noise/capabilities/appstatesync/transport.go`**: interface
`Transport` ganha `StrictSnapshotMAC() bool`.

**`internal/noise/capabilities/appstatesync/fetch.go`**: `ApplyPatches`
passa `t.StrictSnapshotMAC()` a `DecodePatches`.

**`internal/noise/core/client.go`**: campo público
`StrictAppStateSnapshotMAC bool` (padrão `false`), ao lado de
`EmitAppStateEventsOnFullSync` e `AppStateDebugLogs`.

**`internal/noise/core/appstate_transport.go`**: `StrictSnapshotMAC()`
...
- **Controle negativo EXECUTADO** (refeito 2026-08-24 — o primeiro desligava
  ...
  FAIL	wa-api/internal/noise/protocol/appstate	0.188s
  ```
...
**`internal/noise/capabilities/newsletter/queryids.go`**: três query IDs
...
**`internal/noise/capabilities/newsletter/actions.go`**: três funções
...
**`internal/noise/core/newsletter.go`**: três métodos de fachada com
