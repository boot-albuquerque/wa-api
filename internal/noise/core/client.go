// Package noise implements a client for interacting with the WhatsApp web multidevice API.
package core
...
type Client struct {
	Store   *store.Device
	Log     sdklog.Logger
	recvLog sdklog.Logger
	sendLog sdklog.Logger
	...
	// Should noise store recently sent messages in the database so that retry receipts can be accepted
...
//		panic(err)
//	}
//	client := noise.NewClient(deviceStore, nil)
func NewClient(deviceStore *store.Device, log sdklog.Logger) *Client {
	if log == nil {
		log = sdklog.Noop
	}
