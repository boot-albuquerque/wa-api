...
echo "      Binary: $(ls -lh wa-api | awk '{print $5}')"

# 5. Start server
echo "[5/6] Starting server..."
./wa-api --logtype=json --color=false > /tmp/wa-api.log 2>&1 &
SERVER_PID=$!
sleep 3

if ! kill -0 $SERVER_PID 2>/dev/null; then
  echo "ERROR: Server failed to start. Check /tmp/wa-api.log"
  exit 1
fi
...
echo "  Logs: tail -f /tmp/wa-api.log"
echo "========================================"
