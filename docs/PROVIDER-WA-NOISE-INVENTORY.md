# Inventário de capacidades — wa-noise

Levantamento real (código, não teste contra WhatsApp) do que o motor
`wa-noise` implementa hoje, porta a porta, para alimentar a matriz da worktree
...
verificado se algum adapter em `pkg/infra/wa-noise/adapters/*` implementa
...
Rodado `go build ./pkg/infra/wa-noise/... ./pkg/application/...` (limpo),
`go vet ./pkg/infra/wa-noise/...` (limpo) e `go test ./pkg/infra/wa-noise/...`
...
wa-noise, servem qualquer motor). Essas ficam marcadas `N/A` na tabela.

## Tabela: porta → capacidade → status → evidência

| Port (contracts) | capability_name sugerido | Status | Arquivo:linha (evidência) |
|---|---|---|---|
| `JIDResolver` | `resolve_jid` | supported | `pkg/infra/wa-noise/mapping/jid/resolver.go` |
| `PresenceAnnouncer` | `presence_announce` | supported | `pkg/infra/wa-noise/adapters/presence/controller.go:27` (`SendPresence`), `:47` (`SendChatPresence`) |
| `PresenceSubscriber` | `presence_subscribe` | supported | `pkg/infra/wa-noise/adapters/presence/controller.go:60` (`SubscribePresence`) |
| `PresenceController` | `presence_full` | supported | `pkg/infra/wa-noise/adapters/presence/controller.go:73` — `var _ appport.PresenceController = (*PresenceControllerAdapter)(nil)` |
| `ChatMessenger` | `chat_message_ops` (mark-read, react, revoke, edit) | supported | `pkg/infra/wa-noise/adapters/chat/messenger.go:1037` — asserção de compilação |
| `TextMessenger` | `send_text` | supported | `pkg/infra/wa-noise/adapters/chat/messenger.go:242` + `:1038` |
| `MediaMessenger` | `send_media` (image/document/audio/video/sticker) | supported | `pkg/infra/wa-noise/adapters/chat/messenger.go:315,366,417,472,529` + `:1039` |
| `SimpleMessenger` | `send_location` | supported | `pkg/infra/wa-noise/adapters/chat/messenger.go:581` + `:1040` |
...
| `ForwardedMessageSender` | `forward_message` | supported | `pkg/infra/wa-noise/adapters/chat/forward.go:175` — asserção de compilação |
| `StoredMessageReader` | — | N/A | DB (`pkg/infra/db/stored_message_repository.go`), não wa-noise |
| `MediaDownloader` | `download_media` | supported | `pkg/infra/wa-noise/adapters/chat/downloader.go:128` — asserção de compilação |
| `GroupDirectory` | `group_read` (info/link/names/list) | supported | `pkg/infra/wa-noise/adapters/group/adapter.go:36` — asserção de compilação |
| `GroupLifecycle` | `group_create_join_leave` | supported | `pkg/infra/wa-noise/adapters/group/adapter.go:37` |
| `GroupInfoSettings` | `group_settings` (nome/tópico/announce/locked) | supported | `pkg/infra/wa-noise/adapters/group/write.go:60,73,100,113` — parte de `GroupSettings`, asserção em `adapter.go:38` |
| `GroupParticipants` | `group_participants` | supported | `pkg/infra/wa-noise/adapters/group/participants.go:14` |
| `GroupPhotoSetter` | `group_photo` | supported | `pkg/infra/wa-noise/adapters/group/write.go:86` |
| `GroupEphemeralSetter` | `group_disappearing_timer` | supported | `pkg/infra/wa-noise/adapters/group/write.go:126` |
| `GroupSettings` (composição) | `group_settings_full` | supported | `pkg/infra/wa-noise/adapters/group/adapter.go:38` |
| `CommunityDirectory` | `community_read` (`GetSubGroups`, `GetLinkedGroupsParticipants`) | **broken** | código existe em `pkg/infra/wa-noise/adapters/group/community.go:14,30`, asserção de compilação em `adapter.go:40` — MAS `GetSubGroups` e `GetLinkedGroupsParticipants` estão na lista dos 7 métodos sem wrapper de erro (HOUSEKEEP H187): recusa do servidor chega como `500` cru |
...
| `GroupRequests` | `group_join_requests` | supported | `pkg/infra/wa-noise/adapters/group/participants.go:44,57` + `write.go:139` |
| `ChatArchiver` | `chat_archive` | supported | `pkg/infra/wa-noise/adapters/misc/adapter.go:46` |
...
| `SessionCounter` | `count_sessions` | supported | `pkg/infra/wa-noise/adapters/sessioncount/adapter.go:33,52` — asserção de compilação |
| `PhonePairer` | `pair_by_phone` (`IsPaired`, `RequestPairingCode`) | supported | `pkg/infra/wa-noise/adapters/pairing/adapter.go:49,65,88` — asserção de compilação |
| `IdentityResolver` | `identity_lid_pn` (`IsOnWhatsApp`, `GetLIDForPN`, `GetPNForLID`, `GetManyLIDsForPNs`) | supported | `pkg/infra/wa-noise/adapters/user/adapter.go:26,94,119,153` |
...
| `AvatarReader` | `get_avatar` | supported | `pkg/infra/wa-noise/adapters/user/avatar.go:27` |
| `ContactRoster` | `contact_roster` (`GetAllContacts`, `ContactNames`, `GetUserInfo`) | supported | `user/adapter.go:55,80,189` |
| `ContactDirectory` (composição) | `contact_directory_full` | supported | `pkg/infra/wa-noise/adapters/user/adapter.go:178` — asserção de compilação |
| `BlocklistManager` | `blocklist` | supported | `pkg/infra/wa-noise/adapters/user/blocklist.go:17,35` + `adapter.go:179` |
| `PrivacyManager` | `privacy_settings` | supported | `pkg/infra/wa-noise/adapters/user/privacy.go:11,24` + `adapter.go:180` |
| `SessionProvider` / `Session` | `session_lifecycle_provider` | supported | `pkg/infra/wa-noise/runtime/session/provider.go` |
| `SessionGuard` / `SessionDisconnector` / `SessionLogouter` / `SessionController` | `session_control` | supported | `pkg/infra/wa-noise/runtime/session/guard.go` |
| `SessionAttachHook` / `SessionDetacher` | `session_attach_hooks` | supported | `pkg/infra/wa-noise/runtime/session/events.go` |
| `SessionRegistry` | `session_registry` | supported | `pkg/infra/wa-noise/registry/manager.go`, `pkg/infra/wa-noise/registry/clients/clients.go` |
| `ProfileDataAccess` | `profile_data_access` (pushname/ownJID/avatar/contact/device info) | supported | `pkg/infra/wa-noise/adapters/profile/data_access.go:37,45,64,80,104` |
| `DeleteUserProvider` | — | N/A | não é capacidade de protocolo; verificar dono real fora desta worktree (não localizado sob `pkg/infra/wa-noise`) |
| `UserInfoRepublisher` | — | N/A | webhook/republish, não protocolo; implementação fora de `pkg/infra/wa-noise` |
| `MessagingPort` | — | N/A | RabbitMQ, não wa-noise |
...
| `ChatActivityReader` | — | N/A | DB (leitura local, não exige sessão wa-noise) |
| `MediaFetcher` | — | N/A | `pkg/infra/media/opengraph/url_fetcher.go` — genérico, não específico de wa-noise |
...
- **~50 métodos/capacidades wa-noise-relevantes** mapeados (agrupados em ~40
...
- **not_implemented**: nenhuma porta application-level relevante ao motor
  wa-noise ficou sem adapter. A única lacuna de CAPACIDADE real da biblioteca
...
## Diferença Personal vs Business no wa-noise

Evidência encontrada no código (biblioteca vendorizada, não nos adapters
`pkg/`):

1. **`internal/wa-noise/capabilities/user/business.go`** implementa
...
3. **Gap identificado**: `GetBusinessProfile` é uma capacidade REAL da
   biblioteca (`internal/wa-noise/capabilities/user/business.go:72`), mas
   **não há porta em `pkg/application/contracts/` nem adapter em
   `pkg/infra/wa-noise/adapters/` que a exponha** — não localizado nenhuma
...
por tipo de conta (Personal vs Business) nos adapters de `pkg/infra/wa-noise`
ou nos 13 subpacotes de `internal/wa-noise/capabilities/` além do que está
