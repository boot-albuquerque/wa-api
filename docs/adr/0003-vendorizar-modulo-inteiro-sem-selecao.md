# ADR-0003: Vendorizar o módulo wa-noise inteiro (sem seleção de pacotes), `libsignal` externo

- **Status**: accepted (amenda o ADR-0002, aprovado 2026-08-06)
- **Data**: 2026-08-06
- **Amenda**: [ADR-0002](0002-vendorizar-wa-noise-em-vez-de-reimplementar.md),
...
(`.omc/plans/vendor-wa-noise-native-fork.md`), o levantamento técnico
...
1. **O ADR-0002 rejeitou "fork completo ingênuo"** com a justificativa de
   ...
   diretamente `wa-noise/proto/waAdv` e `wa-noise/proto/waCompanionReg`;
   a raiz (`client.go` e afins) importa `appstate` para sync de
   contatos/config — capacidade já em uso pelo `wa-api`
   (`wa-noise/appstate` é importado em 6 arquivos do wa-api hoje — 2 em
   `pkg/bootstrap` e 4 em `pkg/infra/wa-noise`). Ou
...
2. **O ADR-0002 decidiu vendorizar `libsignal` junto** ("vendorizar os
   pacotes de baixo nível do wa-noise ... e o módulo `libsignal`").
   Mas `libsignal` é um **módulo Go publicado separadamente**
   (`go.mau.fi/libsignal`), sem nenhum import reverso para `wa-noise` —
...
1. **Vendorizar o módulo wa-noise inteiro** (raiz + `appstate` + `argo` +
...
- **A alternativa "fork completo ingênuo" do ADR-0002 foi rejeitada por
  ...
  módulo wa-noise já resolve internamente (imports entre `store`,
...
- **`libsignal` nunca foi parte do problema**: o motivador de todo o
  ...
  `wa-noise`. `libsignal` é outro projeto, com seu próprio ciclo de
  release, e vendorizá-lo não muda em nada a dependência do `wa-noise`.
...
- O plano de execução (`.omc/plans/vendor-wa-noise-native-fork.md`)
