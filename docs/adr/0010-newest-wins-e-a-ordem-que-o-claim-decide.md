...
- **Status**: accepted
- **Data**: 2026-08-26
- **Relacionado**: HOUSEKEEP F280 (achado da auditoria adversarial em
  `feature/capability-final-audit`), ADR-0005 (posse de sessão, D1/D2/D3),
  `pkg/infra/db/account_ownership.go`.
...
| proteção estrutural (bypass via SQL cru) | `TestAccountOwnership_StructuralConstraint`, `TestAudit_Invariant3_StructuralIndexRejectsBypassingTheGoLayer` |
| cross-engine não colide | `TestAccountOwnership_CrossEngine`, `TestAudit_AccountOwnership_CrossEngine_Concurrent` |
...
(`pkg/infra/db/account_ownership_audit_test.go`) permanece no repositório
com seu comentário original, que descreve o resultado como "quebra da
invariante 4". Não foi reescrito porque é um registro histórico do processo
de auditoria — o achado que levou a esta ADR — e alterá-lo apagaria essa
trilha. Quem o ler deve ler esta ADR ao lado: o mesmo resultado que aquele
teste chama de "quebrado" é, sob a formalização aqui registrada, o
comportamento pretendido, e `TestNewestWins_ClaimOrderDecidesNotAuthOrder`
é a versão que o afirma como tal.
