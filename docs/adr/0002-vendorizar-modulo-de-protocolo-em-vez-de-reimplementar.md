# ADR-0002: Vendorizar o núcleo do `noise` em vez de reimplementar protocolo do zero
...
- **Amenda**: [ADR-0001](0001-native-whatsapp-protocol-roadmap.md), especificamente a alternativa
  descartada "Fork completo do `noise` agora" e o critério de partida
...
Noise e Signal do zero", e descartou fork completo do `noise` como
...
handshake Noise fazem parte do próprio módulo `wa-api/internal/noise`
...
ratchet) nem é código do `noise` — é o módulo separado
`go.mau.fi/libsignal`, já uma implementação Go independente, pronta pra
vendorizar.

Medição feita nesta sessão (`wa-api/internal/noise v0.0.0-20260516102357-8d3700152a69`,
`go.mau.fi/libsignal v0.2.1`):

| Peça | Linhas | Papel |
|---|---:|---|
| `noise/binary` | 1.223 | encode/decode `waBinary` |
| `noise/socket` | 548 | handshake Noise + framing |
| `libsignal` (módulo separado) | 10.106 | criptografia de sessão (Signal) |
| `noise/store` | 1.156 | persistência de device/chaves |
| `noise/types` | 1.342 | tipos de domínio (JID, eventos) |
| `noise/appstate` | 1.220 | sync de contatos/config |
| `noise` (raiz — `client.go`, `message.go`, `group.go`, `user.go`...) | 14.798 (39 arquivos) | orquestração de alto nível — hoje é o que `SessionProviderAdapter` chama via `noise.Client` |
| `noise/proto` (protobuf gerado — `waE2E`, `waHistorySync` etc.) | 99.205 | serialização dos payloads |
...
`noise` (`binary`, `socket`, `store`, `types`, `proto`) e o módulo
...
`noise.Client` e chamada via `SessionProviderAdapter`) usando essas
...
em `pkg/infra/noise/session_provider_adapter.go` é exatamente o ponto
...
- **Resolve a dor original do ADR-0001** (correções não esperam merge
  ...
  `noise/socket`, só passa a viver no nosso repo em vez de importada
...
- **Fork completo ingênuo (clonar o repo `noise` inteiro, sem
...
- Perde-se atualização automática de segurança/correção do `noise`
...
- Licença MPL-2.0 deve ser respeitada arquivo a arquivo — aviso de
  ...
  origem (commit/tag do `noise` no momento da cópia) para rastreio.
