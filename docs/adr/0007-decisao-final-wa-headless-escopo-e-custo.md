# ADR-0007: Decisão final sobre `wa-headless` — engine, escopo e o limite do que a medição pode decidir

- **Status**: accepted
- **Data**: 2026-08-10
- **Substitui parcialmente**: [ADR-0006](0006-wa-headless-engine-de-browser-e-custo-por-sessao.md),
...
### D2 — `wa-noise` é a pilha primária. `wa-headless` não é caminho geral.

A assimetria de custo é de **duas ordens de grandeza**, e é estrutural:

| | `wa-noise` (protocolo) | `wa-headless` (browser) |
|---|---|---|
...
> `wa-headless` só se constrói contra uma **lista nomeada** de recursos que o
> `wa-noise` não entrega. Essa lista é o business case, e ela precisa justificar
...
Nunca como caminho padrão, nunca no mesmo pool do `wa-noise`. Densidade da ordem
...
**Custos.** `wa-headless` carrega acoplamento estrutural ao SPA da Meta — o
