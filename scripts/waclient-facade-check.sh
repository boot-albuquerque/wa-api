...
#
# Esta checagem falha se qualquer .go FORA de internal/wa-noise/ importar
# wa-api/internal/wa-noise/core. Dentro do fork o import continua livre: o
...
FACADE="internal/wa-noise/main.go"
if [ ! -f "$FACADE" ]; then
  echo "FALHA: $FACADE ausente — a fachada da Fase H (etapa 6) e' o contrato de entrada do fork."
  exit 1
fi

offenders="$(grep -rln '"wa-api/internal/wa-noise/core"' --include='*.go' . \
  | grep -v '^\./internal/wa-noise/' || true)"

if [ -n "$offenders" ]; then
  echo "FALHA: import direto de wa-api/internal/wa-noise/core fora do fork:"
  echo "$offenders" | sed 's/^/  /'
  echo
  echo "core/ e' implementacao. Consumidores importam wa-api/internal/wa-noise"
